load("@prelude//utils:selects.bzl", "selects")

STORE_ABI_VERSION = "pkgs-store-v0"
LOGICAL_STORE_ROOT = "/pkgs/store"
DEFAULT_MAKE_JOBS = 64

_TARGET_SYSTEM = select({
    "//platforms:linux_arm64": "aarch64-linux",
    "//platforms:linux_x86_64": "x86_64-linux",
})

_DYNAMIC_LINKER = select({
    "//platforms:linux_arm64": "lib/ld-linux-aarch64.so.1",
    "//platforms:linux_x86_64": "lib/ld-linux-x86-64.so.2",
})

PkgsPackageInfo = provider(
    fields = [
        "build_closure",
        "foreign_build_entries",
        "foreign_runtime_entries",
        "is_foreign",
        "logical_store_path",
        "name",
        "output",
        "reference_closure",
        "reference_store_outputs",
        "runtime_closure",
        "runtime_store_outputs",
        "store_entry",
        "store_output",
        "store_path_key",
        "tree",
        "tool_closure",
        "tool_store_outputs",
        "validation_outputs",
        "version",
    ],
)

PkgConfigInfo = provider(
    fields = [
        "search_paths",
    ],
)

PkgsOutputSetInfo = provider(
    fields = [
        "outputs",
    ],
)

PkgsStoreSubstituteInfo = provider(
    fields = [
        "archive",
        "logical_store_path",
        "manifest",
        "name",
        "output",
        "references",
        "runtime_store_entries",
        "store_entry",
        "store_name",
        "store_path_key",
        "target_system",
        "tool_store_entries",
        "version",
    ],
)

def pkgs_source(dep, digests = []):
    return struct(
        dep = dep,
        digests = digests,
    )

def pkgs_patch(dep, digests = []):
    return struct(
        dep = dep,
        digests = digests,
    )

def pkgs_http_archive_source(name, sha256, strip_prefix, urls, visibility = []):
    native.http_archive(
        name = name,
        sha256 = sha256,
        strip_prefix = strip_prefix,
        urls = urls,
        visibility = visibility,
    )
    return pkgs_source(
        dep = ":" + name,
        digests = ["sha256:" + sha256],
    )

def pkgs_export_patch(name, src, sha256, visibility = []):
    native.export_file(
        name = name,
        src = src,
        visibility = visibility,
    )
    return pkgs_patch(
        dep = ":" + name,
        digests = ["sha256:" + sha256],
    )

def pkgs_store_path(prefix, dep, suffix = ""):
    return struct(
        _pkgs_configure_value_kind = "store_path",
        dep = dep,
        prefix = prefix,
        suffix = suffix,
    )

def pkgs_self_store_path(prefix, suffix = ""):
    return struct(
        _pkgs_configure_value_kind = "self_store_path",
        prefix = prefix,
        suffix = suffix,
    )

def pkgs_store_path_join(prefix, deps, separator, suffix = ""):
    return struct(
        _pkgs_configure_value_kind = "store_path_join",
        deps = deps,
        prefix = prefix,
        separator = separator,
        suffix = suffix,
    )

def _descriptor_deps(descriptors):
    return [descriptor.dep for descriptor in descriptors]

def _descriptor_digests(descriptors):
    digests = []
    for descriptor in descriptors:
        digests.extend(descriptor.digests)
    return digests

def _select_aware(value, transform):
    return selects.apply(value, transform) if selects.is_select(value) else transform(value)

def _composed_source_digests(sources):
    return [
        "source-layout=compose-sources-v0",
        "source-count={}".format(len(sources)),
    ] + _descriptor_digests(sources)

def _compose_source_set(name, sources):
    source_set_name = "{}__sources".format(name)
    _pkgs_compose_sources(
        name = source_set_name,
        sources = _select_aware(sources, _descriptor_deps),
    )
    return pkgs_source(
        dep = ":" + source_set_name,
        digests = _select_aware(sources, _composed_source_digests),
    )

def _source_set(name, sources):
    if selects.is_select(sources):
        return _compose_source_set(name, sources)

    if len(sources) == 0:
        fail("package {} must declare at least one source".format(name))

    if len(sources) == 1:
        return sources[0]

    return _compose_source_set(name, sources)

def _lower_configure_values(values):
    strings = []
    store_paths = []
    self_store_paths = []
    store_path_joins = []

    for value in values:
        kind = getattr(value, "_pkgs_configure_value_kind", None)
        if kind == None:
            strings.append(value)
        elif kind == "store_path":
            store_paths.append((value.prefix, value.dep, value.suffix))
        elif kind == "self_store_path":
            self_store_paths.append((value.prefix, value.suffix))
        elif kind == "store_path_join":
            store_path_joins.append((value.prefix, value.deps, value.separator, value.suffix))
        else:
            fail("unsupported package configure value kind: {}".format(kind))

    return struct(
        self_store_paths = self_store_paths,
        store_path_joins = store_path_joins,
        store_paths = store_paths,
        strings = strings,
    )

def _store_name(name, version, output):
    base = "{}-{}".format(name, version)
    return base if output == "out" else "{}-{}".format(base, output)

def _package_instance_digest(ctx, deps_by_role, output = None):
    output = ctx.attrs.output if output == None else output
    parts = [
        STORE_ABI_VERSION,
        "name={}".format(ctx.attrs.package_name),
        "version={}".format(ctx.attrs.version),
        "builder={}".format(ctx.attrs.builder),
        "output={}".format(output),
        "target_system={}".format(ctx.attrs.target_system),
    ]
    parts.extend(_recipe_semantic_parts(ctx))
    _append_semantic_values(parts, "source_digest", getattr(ctx.attrs, "source_digests", []))
    _append_semantic_values(parts, "patch_digest", getattr(ctx.attrs, "patch_digests", []))

    for role, deps in deps_by_role:
        parts.append("role={}".format(role))
        for dep in deps:
            parts.append(dep.logical_store_path)

    return sha256("\n".join(parts))

def _append_semantic_values(parts, key, values):
    for value in values:
        parts.append("{}={}".format(key, value))

def _recipe_semantic_parts(ctx):
    parts = []

    _append_semantic_values(parts, "configure_arg", getattr(ctx.attrs, "configure_args", []))
    if getattr(ctx.attrs, "configure_program", "./configure") != "./configure":
        parts.append("configure_program={}".format(ctx.attrs.configure_program))
    if getattr(ctx.attrs, "configure_prefix_arg", "--prefix=") != "--prefix=":
        parts.append("configure_prefix_arg={}".format(ctx.attrs.configure_prefix_arg))
    for prefix, dep, suffix in getattr(ctx.attrs, "configure_arg_store_paths", []):
        parts.append("configure_arg_store_path={}{}{}".format(
            prefix,
            dep[PkgsPackageInfo].logical_store_path,
            suffix,
        ))
    for prefix, suffix in getattr(ctx.attrs, "configure_arg_self_store_paths", []):
        parts.append("configure_arg_self_store_path={}<self>{}".format(prefix, suffix))
    for prefix, deps, separator, suffix in getattr(ctx.attrs, "configure_arg_store_path_joins", []):
        parts.append("configure_arg_store_path_join={}{}{}".format(
            prefix,
            separator.join([dep[PkgsPackageInfo].logical_store_path for dep in deps]),
            suffix,
        ))

    _append_semantic_values(parts, "configure_env", getattr(ctx.attrs, "configure_env", []))
    for prefix, dep, suffix in getattr(ctx.attrs, "configure_env_store_paths", []):
        parts.append("configure_env_store_path={}{}{}".format(
            prefix,
            dep[PkgsPackageInfo].logical_store_path,
            suffix,
        ))
    for prefix, suffix in getattr(ctx.attrs, "configure_env_self_store_paths", []):
        parts.append("configure_env_self_store_path={}<self>{}".format(prefix, suffix))
    for prefix, deps, separator, suffix in getattr(ctx.attrs, "configure_env_store_path_joins", []):
        parts.append("configure_env_store_path_join={}{}{}".format(
            prefix,
            separator.join([dep[PkgsPackageInfo].logical_store_path for dep in deps]),
            suffix,
        ))

    _append_semantic_values(parts, "meson_arg", getattr(ctx.attrs, "meson_args", []))
    for prefix, dep, suffix in getattr(ctx.attrs, "meson_arg_store_paths", []):
        parts.append("meson_arg_store_path={}{}{}".format(
            prefix,
            dep[PkgsPackageInfo].logical_store_path,
            suffix,
        ))
    for prefix, suffix in getattr(ctx.attrs, "meson_arg_self_store_paths", []):
        parts.append("meson_arg_self_store_path={}<self>{}".format(prefix, suffix))
    for prefix, deps, separator, suffix in getattr(ctx.attrs, "meson_arg_store_path_joins", []):
        parts.append("meson_arg_store_path_join={}{}{}".format(
            prefix,
            separator.join([dep[PkgsPackageInfo].logical_store_path for dep in deps]),
            suffix,
        ))

    _append_semantic_values(parts, "meson_env", getattr(ctx.attrs, "meson_env", []))
    for prefix, dep, suffix in getattr(ctx.attrs, "meson_env_store_paths", []):
        parts.append("meson_env_store_path={}{}{}".format(
            prefix,
            dep[PkgsPackageInfo].logical_store_path,
            suffix,
        ))
    for prefix, suffix in getattr(ctx.attrs, "meson_env_self_store_paths", []):
        parts.append("meson_env_self_store_path={}<self>{}".format(prefix, suffix))
    for prefix, deps, separator, suffix in getattr(ctx.attrs, "meson_env_store_path_joins", []):
        parts.append("meson_env_store_path_join={}{}{}".format(
            prefix,
            separator.join([dep[PkgsPackageInfo].logical_store_path for dep in deps]),
            suffix,
        ))
    if hasattr(ctx.attrs, "meson_jobs"):
        parts.append("meson_jobs={}".format(ctx.attrs.meson_jobs))

    if getattr(ctx.attrs, "out_of_source", False):
        parts.append("out_of_source=true")

    _append_semantic_values(parts, "make_arg", getattr(ctx.attrs, "make_args", []))
    for prefix, dep, suffix in getattr(ctx.attrs, "make_arg_store_paths", []):
        parts.append("make_arg_store_path={}{}{}".format(
            prefix,
            dep[PkgsPackageInfo].logical_store_path,
            suffix,
        ))
    for prefix, suffix in getattr(ctx.attrs, "make_arg_self_store_paths", []):
        parts.append("make_arg_self_store_path={}<self>{}".format(prefix, suffix))
    for prefix, deps, separator, suffix in getattr(ctx.attrs, "make_arg_store_path_joins", []):
        parts.append("make_arg_store_path_join={}{}{}".format(
            prefix,
            separator.join([dep[PkgsPackageInfo].logical_store_path for dep in deps]),
            suffix,
        ))
    if hasattr(ctx.attrs, "make_jobs"):
        parts.append("make_jobs={}".format(ctx.attrs.make_jobs))
    _append_semantic_values(parts, "install_arg", getattr(ctx.attrs, "install_args", []))
    for prefix, dep, suffix in getattr(ctx.attrs, "install_arg_store_paths", []):
        parts.append("install_arg_store_path={}{}{}".format(
            prefix,
            dep[PkgsPackageInfo].logical_store_path,
            suffix,
        ))
    for prefix, suffix in getattr(ctx.attrs, "install_arg_self_store_paths", []):
        parts.append("install_arg_self_store_path={}<self>{}".format(prefix, suffix))
    for prefix, deps, separator, suffix in getattr(ctx.attrs, "install_arg_store_path_joins", []):
        parts.append("install_arg_store_path_join={}{}{}".format(
            prefix,
            separator.join([dep[PkgsPackageInfo].logical_store_path for dep in deps]),
            suffix,
        ))
    _append_semantic_values(parts, "python_bytecode_dir", getattr(ctx.attrs, "python_bytecode_dirs", []))
    _append_semantic_values(parts, "python_bytecode_optimize", getattr(ctx.attrs, "python_bytecode_optimizations", []))
    python_bytecode_interpreters = getattr(ctx.attrs, "python_bytecode_interpreters", [])
    if python_bytecode_interpreters:
        parts.append("python_bytecode_interpreter={}/bin/python3".format(
            python_bytecode_interpreters[0][PkgsPackageInfo].logical_store_path,
        ))
    elif getattr(ctx.attrs, "python_bytecode_dirs", []):
        parts.append("python_bytecode_interpreter=<self>/bin/python3")

    if hasattr(ctx.attrs, "patch_strip"):
        parts.append("patch_strip={}".format(ctx.attrs.patch_strip))

    for link, target in sorted(getattr(ctx.attrs, "symlinks", {}).items()):
        parts.append("symlink={}={}".format(link, target))

    for path in sorted(getattr(ctx.attrs, "pkg_config_paths", [])):
        parts.append("pkg_config_path={}".format(path))
    for output, paths in sorted(getattr(ctx.attrs, "split_outputs", {}).items()):
        for path in sorted(paths):
            parts.append("split_output_path={}={}".format(output, path))
    for output, runtime_outputs in sorted(getattr(ctx.attrs, "split_runtime_outputs", {}).items()):
        for runtime_output in sorted(runtime_outputs):
            parts.append("split_runtime_output={}={}".format(output, runtime_output))
    for output, tool_outputs in sorted(getattr(ctx.attrs, "split_tool_outputs", {}).items()):
        for tool_output in sorted(tool_outputs):
            parts.append("split_tool_output={}={}".format(output, tool_output))
    for output, reference_outputs in sorted(getattr(ctx.attrs, "split_reference_outputs", {}).items()):
        for reference_output in sorted(reference_outputs):
            parts.append("split_reference_output={}={}".format(output, reference_output))
    for output, symlinks in sorted(getattr(ctx.attrs, "split_reference_paths", {}).items()):
        for link, target in sorted(symlinks.items()):
            parts.append("split_reference_path={}={}={}".format(output, link, target))
    for output, symlinks in sorted(getattr(ctx.attrs, "split_reference_output_paths", {}).items()):
        for link, target in sorted(symlinks.items()):
            parts.append("split_reference_output_path={}={}={}={}".format(output, link, target[0], target[1]))
    for path in sorted(getattr(ctx.attrs, "output_paths", [])):
        parts.append("output_path={}".format(path))
    for suffix in sorted(getattr(ctx.attrs, "excluded_file_suffixes", [])):
        parts.append("excluded_file_suffix={}".format(suffix))
    if getattr(ctx.attrs, "preserve_debug", False):
        parts.append("preserve_debug=true")
    for path in sorted(getattr(ctx.attrs, "normalize_work_dir_text_paths", [])):
        parts.append("normalize_work_dir_text_path={}".format(path))
    for output, paths in sorted(getattr(ctx.attrs, "split_pkg_config_paths", {}).items()):
        for path in sorted(paths):
            parts.append("split_pkg_config_path={}={}".format(output, path))

    if hasattr(ctx.attrs, "kernel_release"):
        parts.append("kernel_release={}".format(ctx.attrs.kernel_release))

    if hasattr(ctx.attrs, "dynamic_linker"):
        parts.append("dynamic_linker={}".format(ctx.attrs.dynamic_linker))
    for path in sorted(getattr(ctx.attrs, "cxx_include_paths", [])):
        parts.append("cxx_include_path={}".format(path))
    if getattr(ctx.attrs, "suppress_cxx_runtime_rpaths", False):
        parts.append("suppress_cxx_runtime_rpaths=true")

    return parts

def _store_path_parts(ctx, deps_by_role, output = None):
    output = ctx.attrs.output if output == None else output
    store_name = _store_name(
        ctx.attrs.package_name,
        ctx.attrs.version,
        output,
    )
    package_digest = _package_instance_digest(ctx, deps_by_role, output = output)
    store_path_key = sha256(
        "\n".join([
            STORE_ABI_VERSION,
            package_digest,
            output,
            store_name,
        ]),
    )[:32]
    store_entry = "{}-{}".format(store_path_key, store_name)
    return (store_path_key, store_name, store_entry)

def _append_unique(out, seen, entries):
    for entry in entries:
        if entry not in seen:
            seen[entry] = True
            out.append(entry)

def _collect_entries(deps):
    seen = {}
    closure = []

    for dep in deps:
        _append_unique(closure, seen, dep)

    return closure

def _collect_closure(store_entry, deps):
    closure = _collect_entries(deps)
    seen = {entry: True for entry in closure}

    if store_entry not in seen:
        closure.append(store_entry)

    return closure

def _collect_store_outputs(store_entry, store_output, deps, closure_field, outputs_field):
    seen = {}
    outputs = []

    for dep in deps:
        for entry, output in zip(getattr(dep, closure_field), getattr(dep, outputs_field)):
            if entry not in seen:
                seen[entry] = True
                outputs.append(output)

    if store_entry not in seen:
        outputs.append(store_output)

    return outputs

def _collect_runtime_store_outputs(store_entry, store_output, deps):
    return _collect_store_outputs(store_entry, store_output, deps, "runtime_closure", "runtime_store_outputs")

def _collect_reference_store_outputs(store_entry, store_output, deps):
    return _collect_store_outputs(store_entry, store_output, deps, "reference_closure", "reference_store_outputs")

def _collect_tool_store_outputs(store_entry, store_output, deps):
    return _collect_store_outputs(store_entry, store_output, deps, "tool_closure", "tool_store_outputs")

def _logical_store_paths(entries):
    return ["{}/{}".format(LOGICAL_STORE_ROOT, entry) for entry in entries]

def _runtime_manifest_entries(store_entry, deps):
    runtime_closure = _collect_closure(
        store_entry,
        [dep.runtime_closure for dep in deps],
    )
    return runtime_closure

def _canonical_runtime_manifest_entries(store_entry, deps):
    return sorted(_runtime_manifest_entries(store_entry, deps))

def _reference_manifest_entries(store_entry, deps):
    return _collect_closure(
        store_entry,
        [dep.reference_closure for dep in deps],
    )

def _canonical_reference_manifest_entries(store_entry, deps):
    return sorted(_reference_manifest_entries(store_entry, deps))

def _tool_manifest_entries(store_entry, deps):
    return _collect_closure(
        store_entry,
        [dep.tool_closure for dep in deps],
    )

def _canonical_tool_manifest_entries(store_entry, deps):
    return sorted(_tool_manifest_entries(store_entry, deps))

def _store_outputs_for_entries(entries, store_entry, store_output, deps, closure_field, outputs_field):
    outputs_by_entry = {}

    for dep in deps:
        for entry, output in zip(getattr(dep, closure_field), getattr(dep, outputs_field)):
            if entry not in outputs_by_entry:
                outputs_by_entry[entry] = output

    outputs_by_entry[store_entry] = store_output
    return [outputs_by_entry[entry] for entry in entries]

def _runtime_store_outputs_for_entries(entries, store_entry, store_output, deps):
    return _store_outputs_for_entries(entries, store_entry, store_output, deps, "runtime_closure", "runtime_store_outputs")

def _reference_store_outputs_for_entries(entries, store_entry, store_output, deps):
    return _store_outputs_for_entries(entries, store_entry, store_output, deps, "reference_closure", "reference_store_outputs")

def _tool_store_outputs_for_entries(entries, store_entry, store_output, deps):
    return _store_outputs_for_entries(entries, store_entry, store_output, deps, "tool_closure", "tool_store_outputs")

def _deps_by_role(ctx):
    deps_by_role = [
        ("native_build_inputs", [dep[PkgsPackageInfo] for dep in ctx.attrs.native_build_inputs]),
        ("build_inputs", [dep[PkgsPackageInfo] for dep in ctx.attrs.build_inputs]),
        ("target_inputs", [dep[PkgsPackageInfo] for dep in ctx.attrs.target_inputs]),
        ("runtime_inputs", [dep[PkgsPackageInfo] for dep in ctx.attrs.runtime_inputs]),
    ]
    reference_inputs = getattr(ctx.attrs, "reference_inputs", [])
    if reference_inputs:
        deps_by_role.append(("reference_inputs", [dep[PkgsPackageInfo] for dep in reference_inputs]))
    tool_inputs = getattr(ctx.attrs, "tool_inputs", [])
    if tool_inputs:
        deps_by_role.append(("tool_inputs", [dep[PkgsPackageInfo] for dep in tool_inputs]))
    for output, deps in sorted(getattr(ctx.attrs, "split_runtime_inputs", {}).items()):
        deps_by_role.append(("split_runtime_inputs[{}]".format(output), [dep[PkgsPackageInfo] for dep in deps]))
    for output, deps in sorted(getattr(ctx.attrs, "split_reference_inputs", {}).items()):
        deps_by_role.append(("split_reference_inputs[{}]".format(output), [dep[PkgsPackageInfo] for dep in deps]))
    for output, deps in sorted(getattr(ctx.attrs, "split_tool_inputs", {}).items()):
        deps_by_role.append(("split_tool_inputs[{}]".format(output), [dep[PkgsPackageInfo] for dep in deps]))
    link_inputs = getattr(ctx.attrs, "link_inputs", [])
    if link_inputs:
        deps_by_role.append(("link_inputs", [dep[PkgsPackageInfo] for dep in link_inputs]))
    link_interface_inputs = getattr(ctx.attrs, "link_interface_inputs", [])
    if link_interface_inputs:
        deps_by_role.append(("link_interface_inputs", [dep[PkgsPackageInfo] for dep in link_interface_inputs]))
    python_bytecode_interpreters = getattr(ctx.attrs, "python_bytecode_interpreters", [])
    if python_bytecode_interpreters:
        deps_by_role.append(("python_bytecode_interpreters", [dep[PkgsPackageInfo] for dep in python_bytecode_interpreters]))
    return deps_by_role

def _package_metadata(ctx, output = None, runtime_deps = None, runtime_infos = None, reference_infos = None, tool_infos = None):
    output = ctx.attrs.output if output == None else output
    deps_by_role = _deps_by_role(ctx)
    dependency_infos = [dep for _, deps in deps_by_role for dep in deps]
    store_path_key, store_name, store_entry = _store_path_parts(ctx, deps_by_role, output = output)
    logical_store_path = "{}/{}".format(LOGICAL_STORE_ROOT, store_entry)
    store_path = ctx.actions.store_path(
        store_path_key = store_path_key,
        store_name = store_name,
    )
    store_output = ctx.actions.declare_store_output(
        store_path = store_path,
        dir = True,
    )
    build_closure = _collect_closure(
        store_entry,
        [dep.build_closure for dep in dependency_infos],
    )
    if runtime_infos == None:
        if runtime_deps == None:
            runtime_deps = [dep for dep in ctx.attrs.runtime_inputs] + getattr(ctx.attrs, "link_inputs", [])
        runtime_infos = [dep[PkgsPackageInfo] for dep in runtime_deps]
    runtime_closure = _collect_closure(
        store_entry,
        [dep.runtime_closure for dep in runtime_infos],
    )
    runtime_store_outputs = _collect_runtime_store_outputs(
        store_entry,
        store_output,
        runtime_infos,
    )
    if tool_infos == None:
        tool_infos = runtime_infos + [dep[PkgsPackageInfo] for dep in getattr(ctx.attrs, "tool_inputs", [])]
    tool_closure = _collect_closure(
        store_entry,
        [dep.tool_closure for dep in tool_infos],
    )
    tool_store_outputs = _collect_tool_store_outputs(
        store_entry,
        store_output,
        tool_infos,
    )
    if reference_infos == None:
        reference_infos = runtime_infos + tool_infos + [dep[PkgsPackageInfo] for dep in getattr(ctx.attrs, "reference_inputs", [])]
    reference_closure = _collect_closure(
        store_entry,
        [dep.reference_closure for dep in reference_infos],
    )
    reference_store_outputs = _collect_reference_store_outputs(
        store_entry,
        store_output,
        reference_infos,
    )
    foreign_runtime_entries = _collect_entries(
        [dep.foreign_runtime_entries for dep in runtime_infos],
    )
    foreign_build_entries = _collect_entries(
        [dep.foreign_build_entries for dep in dependency_infos],
    )
    validation_outputs = [
        output
        for dep in dependency_infos
        for output in dep.validation_outputs
    ]
    if ctx.attrs.foreign:
        foreign_build_entries = _collect_closure(store_entry, [foreign_build_entries])
        foreign_runtime_entries = _collect_closure(store_entry, [foreign_runtime_entries])

    return struct(
        build_closure = build_closure,
        foreign_build_entries = foreign_build_entries,
        foreign_runtime_entries = foreign_runtime_entries,
        logical_store_path = logical_store_path,
        reference_closure = reference_closure,
        reference_store_outputs = reference_store_outputs,
        runtime_closure = runtime_closure,
        runtime_store_outputs = runtime_store_outputs,
        store_entry = store_entry,
        store_output = store_output,
        store_path_key = store_path_key,
        tool_closure = tool_closure,
        tool_store_outputs = tool_store_outputs,
        validation_outputs = validation_outputs,
    )

def _split_output_metadata(ctx, primary_metadata):
    primary_output = ctx.attrs.output
    primary_package = _package_info(ctx, primary_metadata.store_output, primary_metadata, primary_output)
    split_runtime_outputs = ctx.attrs.split_runtime_outputs
    split_tool_outputs = ctx.attrs.split_tool_outputs
    split_reference_outputs = ctx.attrs.split_reference_outputs
    split_reference_output_paths = ctx.attrs.split_reference_output_paths

    for output in split_runtime_outputs:
        if output not in ctx.attrs.split_outputs:
            fail("package {} declares runtime outputs for unknown split output {}".format(ctx.label, output))
    for output in split_tool_outputs:
        if output not in ctx.attrs.split_outputs:
            fail("package {} declares tool outputs for unknown split output {}".format(ctx.label, output))
    for output in split_reference_outputs:
        if output not in ctx.attrs.split_outputs:
            fail("package {} declares reference outputs for unknown split output {}".format(ctx.label, output))
    for output in split_reference_output_paths:
        if output not in ctx.attrs.split_outputs:
            fail("package {} declares reference projection paths for unknown split output {}".format(ctx.label, output))

    split_metadata = {}
    packages = {primary_output: primary_package}
    outputs = [output for output in ctx.attrs.split_outputs]
    global_tool_infos = [dep[PkgsPackageInfo] for dep in getattr(ctx.attrs, "tool_inputs", [])]
    global_reference_infos = [dep[PkgsPackageInfo] for dep in getattr(ctx.attrs, "reference_inputs", [])]

    for _ in outputs:
        for output in outputs:
            if output in split_metadata:
                continue

            runtime_outputs = split_runtime_outputs.get(output, [])
            tool_outputs = split_tool_outputs.get(output, [])
            reference_outputs = split_reference_outputs.get(output, [])
            if output in ctx.attrs.split_reference_paths and primary_output not in reference_outputs:
                reference_outputs = reference_outputs + [primary_output]
            for _, target in split_reference_output_paths.get(output, {}).items():
                if target[0] not in reference_outputs:
                    reference_outputs = reference_outputs + [target[0]]

            declared_outputs = runtime_outputs + tool_outputs + reference_outputs
            for dependency_output in declared_outputs:
                if dependency_output != primary_output and dependency_output not in ctx.attrs.split_outputs:
                    fail("package {} split output {} names unknown dependent output {}".format(ctx.label, output, dependency_output))
            if any([dependency_output not in packages for dependency_output in declared_outputs]):
                continue

            runtime_infos = [dep[PkgsPackageInfo] for dep in ctx.attrs.split_runtime_inputs.get(output, [])] + [
                packages[runtime_output]
                for runtime_output in runtime_outputs
            ]
            tool_infos = runtime_infos + global_tool_infos + [dep[PkgsPackageInfo] for dep in ctx.attrs.split_tool_inputs.get(output, [])] + [
                packages[tool_output]
                for tool_output in tool_outputs
            ]
            reference_infos = runtime_infos + tool_infos + global_reference_infos + [dep[PkgsPackageInfo] for dep in ctx.attrs.split_reference_inputs.get(output, [])] + [
                packages[reference_output]
                for reference_output in reference_outputs
            ]
            split_metadata[output] = _package_metadata(
                ctx,
                output = output,
                runtime_infos = runtime_infos,
                reference_infos = reference_infos,
                tool_infos = tool_infos,
            )
            packages[output] = _package_info(ctx, split_metadata[output].store_output, split_metadata[output], output)

    if len(split_metadata) != len(outputs):
        unresolved = [output for output in outputs if output not in split_metadata]
        fail("package {} split output graph contains a cycle involving {}".format(ctx.label, unresolved))
    return split_metadata

def _declare_reproducibility_stamp(ctx, metadata, replay_tree, output = None):
    stem = ctx.label.name if output == None else "{}.{}".format(ctx.label.name, output)
    stamp = ctx.actions.declare_output("{}.reproducible".format(stem))
    ctx.actions.run(
        cmd_args([
            ctx.attrs._reproducibility_verifier[RunInfo],
            "--expected",
            metadata.store_output,
            "--actual",
            replay_tree,
            "--stamp",
            stamp.as_output(),
        ]),
        category = "pkgs_verify_reproducible_tree",
        identifier = stem,
    )
    return stamp

def _declare_archive_metadata_stamp(ctx, metadata, output = None):
    stem = ctx.label.name if output == None else "{}.{}".format(ctx.label.name, output)
    stamp = ctx.actions.declare_output("{}.archive_metadata".format(stem))
    ctx.actions.run(
        cmd_args([
            ctx.attrs._archive_metadata_verifier[RunInfo],
            "--input",
            metadata.store_output,
            "--stamp",
            stamp.as_output(),
        ]),
        category = "pkgs_verify_archive_metadata",
        identifier = stem,
    )
    return stamp

def _package_sub_targets(reproducibility_stamp, archive_metadata_stamp):
    return {
        "archive_metadata": [DefaultInfo(default_output = archive_metadata_stamp)],
        "reproducible": [DefaultInfo(default_output = reproducibility_stamp)],
    }

def _pkg_config_provider_for_path(logical_store_path, paths):
    if not paths:
        return None
    return PkgConfigInfo(
        search_paths = [
            "{}/{}".format(logical_store_path, path)
            for path in paths
        ],
    )

def _pkg_config_provider(metadata, paths):
    return _pkg_config_provider_for_path(metadata.logical_store_path, paths)

def _package_info(ctx, tree, metadata, output):
    return PkgsPackageInfo(
        build_closure = metadata.build_closure,
        foreign_build_entries = metadata.foreign_build_entries,
        foreign_runtime_entries = metadata.foreign_runtime_entries,
        is_foreign = ctx.attrs.foreign,
        logical_store_path = metadata.logical_store_path,
        name = ctx.attrs.package_name,
        output = output,
        reference_closure = metadata.reference_closure,
        reference_store_outputs = metadata.reference_store_outputs,
        runtime_closure = metadata.runtime_closure,
        runtime_store_outputs = metadata.runtime_store_outputs,
        store_entry = metadata.store_entry,
        store_output = metadata.store_output,
        store_path_key = metadata.store_path_key,
        tree = tree,
        tool_closure = metadata.tool_closure,
        tool_store_outputs = metadata.tool_store_outputs,
        validation_outputs = metadata.validation_outputs,
        version = ctx.attrs.version,
    )

def _package_result(
        ctx,
        tree,
        metadata,
        reproducibility_stamp,
        archive_metadata_stamp,
        other_outputs = [],
        output_records = {}):
    result = [
        DefaultInfo(
            default_output = metadata.store_output,
            other_outputs = other_outputs,
            sub_targets = _package_sub_targets(reproducibility_stamp, archive_metadata_stamp),
        ),
        _package_info(ctx, tree, metadata, ctx.attrs.output),
    ]
    pkg_config = _pkg_config_provider(metadata, getattr(ctx.attrs, "pkg_config_paths", []))
    if pkg_config != None:
        result.append(pkg_config)
    if output_records:
        result.append(PkgsOutputSetInfo(outputs = output_records))
    return result

def _pkgs_package_output_impl(ctx):
    record = ctx.attrs.package[PkgsOutputSetInfo].outputs[ctx.attrs.output]
    result = [
        DefaultInfo(
            default_output = record.package.store_output,
            sub_targets = _package_sub_targets(record.reproducibility_stamp, record.archive_metadata_stamp),
        ),
        record.package,
    ]
    if record.pkg_config != None:
        result.append(record.pkg_config)
    return result

_pkgs_package_output = rule(
    impl = _pkgs_package_output_impl,
    attrs = {
        "output": attrs.string(),
        "package": attrs.dep(providers = [PkgsOutputSetInfo]),
    },
)

def pkgs_package_output(name, package, output, visibility = []):
    _pkgs_package_output(
        name = name,
        package = package,
        output = output,
        visibility = visibility,
    )

def _stage_tree_package(ctx, tree):
    metadata = _package_metadata(ctx)

    ctx.actions.run(
        cmd_args(
            [
                ctx.attrs._tree_stager[RunInfo],
                "--source",
                tree,
                "--output",
                metadata.store_output.as_output(),
            ],
            hidden = [
                output
                for dep in ctx.attrs.runtime_inputs
                for output in dep[PkgsPackageInfo].runtime_store_outputs
            ],
        ),
        category = "pkgs_stage_tree",
        identifier = ctx.attrs.package_name,
    )

    replay_tree = ctx.actions.declare_output("{}.replay".format(ctx.label.name), dir = True)
    ctx.actions.run(
        cmd_args([
            ctx.attrs._tree_stager[RunInfo],
            "--source",
            tree,
            "--output",
            replay_tree.as_output(),
        ]),
        category = "pkgs_replay_stage_tree",
        identifier = ctx.attrs.package_name,
    )
    reproducibility_stamp = _declare_reproducibility_stamp(ctx, metadata, replay_tree)
    archive_metadata_stamp = _declare_archive_metadata_stamp(ctx, metadata)

    return _package_result(
        ctx,
        metadata.store_output,
        metadata,
        reproducibility_stamp,
        archive_metadata_stamp,
        other_outputs = [tree],
    )

def _pkgs_package_impl(ctx):
    return _stage_tree_package(ctx, ctx.attrs.src[DefaultInfo].default_outputs[0])

def _pkgs_compose_sources_impl(ctx):
    output = ctx.actions.declare_output("{}.sources".format(ctx.label.name), dir = True)
    args = cmd_args([
        ctx.attrs._composer[RunInfo],
        "--output",
        output.as_output(),
    ])
    for source in ctx.attrs.sources:
        args.add("--source", source[DefaultInfo].default_outputs[0])

    ctx.actions.run(
        args,
        category = "pkgs_compose_sources",
        identifier = ctx.label.name,
    )

    return [DefaultInfo(default_output = output)]

_pkgs_compose_sources = rule(
    impl = _pkgs_compose_sources_impl,
    attrs = {
        "sources": attrs.list(attrs.dep(providers = [DefaultInfo])),
        "_composer": attrs.default_only(
            attrs.exec_dep(
                default = "//crates/pkgs-tool:pkgs_compose_sources",
                providers = [RunInfo],
            ),
        ),
    },
)

_pkgs_package = rule(
    impl = _pkgs_package_impl,
    attrs = {
        "build_inputs": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "builder": attrs.string(),
        "foreign": attrs.bool(default = False),
        "native_build_inputs": attrs.list(attrs.exec_dep(providers = [PkgsPackageInfo]), default = []),
        "output": attrs.string(default = "out"),
        "package_name": attrs.string(),
        "pkg_config_paths": attrs.list(attrs.string(), default = []),
        "reference_inputs": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "runtime_inputs": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "source_digests": attrs.list(attrs.string(), default = []),
        "src": attrs.dep(providers = [DefaultInfo]),
        "target_system": attrs.string(),
        "target_inputs": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "tool_inputs": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "version": attrs.string(),
        "_tree_stager": attrs.default_only(
            attrs.exec_dep(
                default = "//crates/pkgs-tool:pkgs_stage_tree",
                providers = [RunInfo],
            ),
        ),
        "_reproducibility_verifier": attrs.default_only(
            attrs.exec_dep(
                default = "//crates/pkgs-tool:pkgs_verify_reproducible_tree",
                providers = [RunInfo],
            ),
        ),
        "_archive_metadata_verifier": attrs.default_only(
            attrs.exec_dep(
                default = "//crates/pkgs-tool:pkgs_verify_archive_metadata",
                providers = [RunInfo],
            ),
        ),
    },
)

def pkgs_package(
        name,
        package_name,
        version,
        builder,
        sources,
        output = "out",
        foreign = False,
        native_build_inputs = [],
        build_inputs = [],
        target_inputs = [],
        runtime_inputs = [],
        reference_inputs = [],
        tool_inputs = [],
        visibility = [],
        pkg_config_paths = []):
    source = _source_set(name, sources)
    _pkgs_package(
        name = name,
        package_name = package_name,
        version = version,
        output = output,
        builder = builder,
        foreign = foreign,
        pkg_config_paths = pkg_config_paths,
        source_digests = source.digests,
        src = source.dep,
        target_system = _TARGET_SYSTEM,
        native_build_inputs = native_build_inputs,
        build_inputs = build_inputs,
        target_inputs = target_inputs,
        reference_inputs = reference_inputs,
        runtime_inputs = runtime_inputs,
        tool_inputs = tool_inputs,
        visibility = visibility,
    )

def _declare_store_substitute(
        ctx,
        tree,
        store_path_key,
        store_name,
        package_name,
        version,
        output,
        target_system,
        reference_store_entries,
        runtime_store_entries,
        tool_store_entries,
        validation_outputs = []):
    reference_store_entries = sorted(_collect_entries([reference_store_entries]))
    runtime_store_entries = sorted(_collect_entries([runtime_store_entries]))
    tool_store_entries = sorted(_collect_entries([tool_store_entries]))
    store_entry = "{}-{}".format(store_path_key, store_name)
    logical_store_path = "{}/{}".format(LOGICAL_STORE_ROOT, store_entry)
    archive = ctx.actions.declare_output("{}.bpkgs-tree".format(ctx.label.name))
    manifest = ctx.actions.declare_output("{}.manifest.json".format(ctx.label.name))
    references = [
        entry
        for entry in reference_store_entries
        if entry != store_entry
    ]
    args = cmd_args(
        [
            ctx.attrs._exporter[RunInfo],
            "--input",
            tree,
            "--store-path",
            logical_store_path,
            "--store-path-key",
            store_path_key,
            "--store-entry",
            store_entry,
            "--package-name",
            package_name,
            "--version",
            version,
            "--output",
            output,
            "--target-system",
            target_system,
            "--archive",
            archive.as_output(),
            "--manifest",
            manifest.as_output(),
        ],
        hidden = validation_outputs,
    )
    for entry in references:
        args.add("--reference", "{}/{}".format(LOGICAL_STORE_ROOT, entry))
    for entry in runtime_store_entries:
        args.add("--runtime-store-output", "{}/{}".format(LOGICAL_STORE_ROOT, entry))
    for entry in tool_store_entries:
        args.add("--tool-store-output", "{}/{}".format(LOGICAL_STORE_ROOT, entry))

    ctx.actions.run(
        args,
        category = "pkgs_export_store_substitute",
        identifier = ctx.label.name,
    )

    return [
        DefaultInfo(default_output = manifest, other_outputs = [archive]),
        PkgsStoreSubstituteInfo(
            archive = archive,
            logical_store_path = logical_store_path,
            manifest = manifest,
            name = package_name,
            output = output,
            references = _logical_store_paths(references),
            runtime_store_entries = runtime_store_entries,
            store_entry = store_entry,
            store_name = store_name,
            store_path_key = store_path_key,
            target_system = target_system,
            tool_store_entries = tool_store_entries,
            version = version,
        ),
    ]

def _pkgs_export_store_substitute_impl(ctx):
    package = ctx.attrs.package[PkgsPackageInfo]
    return _declare_store_substitute(
        ctx,
        package.store_output,
        package.store_path_key,
        package.store_entry[len(package.store_path_key) + 1:],
        package.name,
        package.version,
        package.output,
        ctx.attrs.target_system,
        package.reference_closure,
        package.runtime_closure,
        package.tool_closure,
        package.validation_outputs,
    )

_pkgs_export_store_substitute = rule(
    impl = _pkgs_export_store_substitute_impl,
    attrs = {
        "package": attrs.dep(providers = [PkgsPackageInfo]),
        "target_system": attrs.string(),
        "_exporter": attrs.default_only(
            attrs.exec_dep(
                default = "//crates/pkgs-tool:pkgs_export_store_object",
                providers = [RunInfo],
            ),
        ),
    },
)

def pkgs_export_store_substitute(name, package, visibility = []):
    _pkgs_export_store_substitute(
        name = name,
        package = package,
        target_system = _TARGET_SYSTEM,
        visibility = visibility,
    )

def _pkgs_export_store_closure_impl(ctx):
    output = ctx.actions.declare_output("{}.bundle".format(ctx.label.name), dir = True)
    args = cmd_args([
        ctx.attrs._exporter[RunInfo],
        "--name",
        ctx.attrs.closure_name,
        "--target-system",
        ctx.attrs.target_system,
        "--output",
        output.as_output(),
    ])
    for root in ctx.attrs.roots:
        args.add("--root", root[PkgsStoreSubstituteInfo].logical_store_path)
    for object in ctx.attrs.objects:
        substitute = object[PkgsStoreSubstituteInfo]
        args.add("--object-manifest", substitute.manifest)
        args.add("--object-archive", substitute.archive)
    ctx.actions.run(
        args,
        category = "pkgs_export_store_closure",
        identifier = ctx.label.name,
    )
    return [DefaultInfo(default_output = output)]

_pkgs_export_store_closure = rule(
    impl = _pkgs_export_store_closure_impl,
    attrs = {
        "closure_name": attrs.string(),
        "objects": attrs.list(attrs.dep(providers = [PkgsStoreSubstituteInfo])),
        "roots": attrs.list(attrs.dep(providers = [PkgsStoreSubstituteInfo])),
        "target_system": attrs.string(),
        "_exporter": attrs.default_only(
            attrs.exec_dep(
                default = "//crates/pkgs-tool:pkgs_export_store_closure",
                providers = [RunInfo],
            ),
        ),
    },
)

def pkgs_export_store_closure(name, closure_name, roots, objects, visibility = []):
    _pkgs_export_store_closure(
        name = name,
        closure_name = closure_name,
        roots = roots,
        objects = objects,
        target_system = _TARGET_SYSTEM,
        visibility = visibility,
    )

def _pkgs_export_store_tree_substitute_impl(ctx):
    store_entry = "{}-{}".format(ctx.attrs.store_path_key, ctx.attrs.store_name)
    runtime_store_entries = _collect_closure(store_entry, [ctx.attrs.runtime_store_entries])
    reference_store_entries = _collect_closure(store_entry, [ctx.attrs.reference_store_entries])
    tool_store_entries = _collect_closure(store_entry, [ctx.attrs.tool_store_entries])
    return _declare_store_substitute(
        ctx,
        ctx.attrs.tree[DefaultInfo].default_outputs[0],
        ctx.attrs.store_path_key,
        ctx.attrs.store_name,
        ctx.attrs.package_name,
        ctx.attrs.version,
        ctx.attrs.output,
        ctx.attrs.target_system,
        reference_store_entries,
        runtime_store_entries,
        tool_store_entries,
    )

_pkgs_export_store_tree_substitute = rule(
    impl = _pkgs_export_store_tree_substitute_impl,
    attrs = {
        "output": attrs.string(default = "out"),
        "package_name": attrs.string(),
        "reference_store_entries": attrs.list(attrs.string(), default = []),
        "runtime_store_entries": attrs.list(attrs.string(), default = []),
        "store_name": attrs.string(),
        "store_path_key": attrs.string(),
        "target_system": attrs.string(),
        "tool_store_entries": attrs.list(attrs.string(), default = []),
        "tree": attrs.dep(providers = [DefaultInfo]),
        "version": attrs.string(),
        "_exporter": attrs.default_only(
            attrs.exec_dep(
                default = "//crates/pkgs-tool:pkgs_export_store_object",
                providers = [RunInfo],
            ),
        ),
    },
)

def pkgs_export_store_tree_substitute(
        name,
        tree,
        package_name,
        version,
        store_path_key,
        store_name,
        output = "out",
        reference_store_entries = [],
        runtime_store_entries = [],
        tool_store_entries = [],
        visibility = []):
    _pkgs_export_store_tree_substitute(
        name = name,
        tree = tree,
        package_name = package_name,
        version = version,
        output = output,
        store_path_key = store_path_key,
        store_name = store_name,
        target_system = _TARGET_SYSTEM,
        reference_store_entries = runtime_store_entries if not reference_store_entries else reference_store_entries,
        runtime_store_entries = runtime_store_entries,
        tool_store_entries = runtime_store_entries if not tool_store_entries else tool_store_entries,
        visibility = visibility,
    )

def _pkgs_prebuilt_store_substitute_impl(ctx):
    store_entry = "{}-{}".format(ctx.attrs.store_path_key, ctx.attrs.store_name)
    return [
        DefaultInfo(
            default_output = ctx.attrs.manifest[DefaultInfo].default_outputs[0],
            other_outputs = [ctx.attrs.archive[DefaultInfo].default_outputs[0]],
        ),
        PkgsStoreSubstituteInfo(
            archive = ctx.attrs.archive[DefaultInfo].default_outputs[0],
            logical_store_path = "{}/{}".format(LOGICAL_STORE_ROOT, store_entry),
            manifest = ctx.attrs.manifest[DefaultInfo].default_outputs[0],
            name = ctx.attrs.package_name,
            output = ctx.attrs.output,
            references = ctx.attrs.references,
            runtime_store_entries = ctx.attrs.runtime_store_entries,
            store_entry = store_entry,
            store_name = ctx.attrs.store_name,
            store_path_key = ctx.attrs.store_path_key,
            target_system = ctx.attrs.target_system,
            tool_store_entries = ctx.attrs.tool_store_entries if ctx.attrs.tool_store_entries else ctx.attrs.runtime_store_entries,
            version = ctx.attrs.version,
        ),
    ]

_pkgs_prebuilt_store_substitute = rule(
    impl = _pkgs_prebuilt_store_substitute_impl,
    attrs = {
        "archive": attrs.dep(providers = [DefaultInfo]),
        "manifest": attrs.dep(providers = [DefaultInfo]),
        "output": attrs.string(default = "out"),
        "package_name": attrs.string(),
        "references": attrs.list(attrs.string(), default = []),
        "runtime_store_entries": attrs.list(attrs.string(), default = []),
        "store_name": attrs.string(),
        "store_path_key": attrs.string(),
        "target_system": attrs.string(),
        "tool_store_entries": attrs.list(attrs.string(), default = []),
        "version": attrs.string(),
    },
)

def pkgs_prebuilt_store_substitute(
        name,
        archive,
        manifest,
        package_name,
        version,
        store_path_key,
        store_name,
        target_system,
        output = "out",
        references = [],
        runtime_store_entries = [],
        tool_store_entries = [],
        visibility = []):
    _pkgs_prebuilt_store_substitute(
        name = name,
        archive = archive,
        manifest = manifest,
        package_name = package_name,
        version = version,
        output = output,
        store_path_key = store_path_key,
        store_name = store_name,
        target_system = target_system,
        references = references,
        runtime_store_entries = runtime_store_entries,
        tool_store_entries = tool_store_entries,
        visibility = visibility,
    )

def _pkgs_imported_store_output_impl(ctx):
    substitute = ctx.attrs.substitute[PkgsStoreSubstituteInfo]
    runtime_inputs = [dep[PkgsPackageInfo] for dep in ctx.attrs.runtime_inputs]
    tool_inputs = runtime_inputs + [dep[PkgsPackageInfo] for dep in ctx.attrs.tool_inputs]
    reference_inputs = tool_inputs + [dep[PkgsPackageInfo] for dep in ctx.attrs.reference_inputs]
    runtime_closure = _canonical_runtime_manifest_entries(substitute.store_entry, runtime_inputs)
    tool_closure = _canonical_tool_manifest_entries(substitute.store_entry, tool_inputs)
    reference_closure = _canonical_reference_manifest_entries(substitute.store_entry, reference_inputs)
    references = [entry for entry in reference_closure if entry != substitute.store_entry]
    if runtime_closure != substitute.runtime_store_entries:
        fail("imported package runtime closure does not match substitute manifest metadata for {}".format(substitute.logical_store_path))
    if tool_closure != substitute.tool_store_entries:
        fail("imported package tool-use closure does not match substitute manifest metadata for {}".format(substitute.logical_store_path))
    if _logical_store_paths(references) != substitute.references:
        fail("imported package references do not match substitute manifest metadata for {}".format(substitute.logical_store_path))

    store_path = ctx.actions.store_path(
        store_path_key = substitute.store_path_key,
        store_name = substitute.store_name,
    )
    store_output = ctx.actions.declare_store_output(
        store_path = store_path,
        dir = True,
    )
    runtime_store_outputs = _runtime_store_outputs_for_entries(
        runtime_closure,
        substitute.store_entry,
        store_output,
        runtime_inputs,
    )
    tool_store_outputs = _tool_store_outputs_for_entries(
        tool_closure,
        substitute.store_entry,
        store_output,
        tool_inputs,
    )
    reference_store_outputs = _reference_store_outputs_for_entries(
        reference_closure,
        substitute.store_entry,
        store_output,
        reference_inputs,
    )
    args = cmd_args([
        ctx.attrs._importer[RunInfo],
        "--manifest",
        substitute.manifest,
        "--archive",
        substitute.archive,
        "--expected-store-path",
        substitute.logical_store_path,
        "--expected-package-name",
        substitute.name,
        "--expected-version",
        substitute.version,
        "--expected-output",
        substitute.output,
        "--expected-target-system",
        substitute.target_system,
        "--output",
        store_output.as_output(),
    ])
    for reference in substitute.references:
        args.add("--expected-reference", reference)
    for entry in substitute.runtime_store_entries:
        args.add("--expected-runtime-store-output", "{}/{}".format(LOGICAL_STORE_ROOT, entry))
    for entry in substitute.tool_store_entries:
        args.add("--expected-tool-store-output", "{}/{}".format(LOGICAL_STORE_ROOT, entry))
    ctx.actions.run(
        args,
        category = "pkgs_import_store_object",
        identifier = ctx.label.name,
    )

    result = [
        DefaultInfo(default_output = store_output),
        PkgsPackageInfo(
            build_closure = tool_closure,
            foreign_build_entries = [],
            foreign_runtime_entries = [],
            is_foreign = False,
            logical_store_path = substitute.logical_store_path,
            name = substitute.name,
            output = substitute.output,
            reference_closure = reference_closure,
            reference_store_outputs = reference_store_outputs,
            runtime_closure = runtime_closure,
            runtime_store_outputs = runtime_store_outputs,
            store_entry = substitute.store_entry,
            store_output = store_output,
            store_path_key = substitute.store_path_key,
            tree = store_output,
            tool_closure = tool_closure,
            tool_store_outputs = tool_store_outputs,
            validation_outputs = [],
            version = substitute.version,
        ),
    ]
    pkg_config = _pkg_config_provider_for_path(substitute.logical_store_path, ctx.attrs.pkg_config_paths)
    if pkg_config != None:
        result.append(pkg_config)
    return result

_pkgs_imported_store_output = rule(
    impl = _pkgs_imported_store_output_impl,
    attrs = {
        "pkg_config_paths": attrs.list(attrs.string(), default = []),
        "reference_inputs": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "runtime_inputs": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "substitute": attrs.dep(providers = [PkgsStoreSubstituteInfo]),
        "tool_inputs": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "_importer": attrs.default_only(
            attrs.exec_dep(
                default = "//crates/pkgs-tool:pkgs_import_store_object",
                providers = [RunInfo],
            ),
        ),
    },
)

def pkgs_imported_store_output(name, substitute, runtime_inputs = [], reference_inputs = [], tool_inputs = [], visibility = [], pkg_config_paths = []):
    _pkgs_imported_store_output(
        name = name,
        substitute = substitute,
        pkg_config_paths = pkg_config_paths,
        reference_inputs = reference_inputs,
        runtime_inputs = runtime_inputs,
        tool_inputs = tool_inputs,
        visibility = visibility,
    )

def _pinned_import_metadata(ctx, store_entry, logical_store_path):
    runtime_inputs = [dep[PkgsPackageInfo] for dep in ctx.attrs.runtime_inputs]
    tool_inputs = runtime_inputs + [dep[PkgsPackageInfo] for dep in ctx.attrs.tool_inputs]
    reference_inputs = tool_inputs + [dep[PkgsPackageInfo] for dep in ctx.attrs.reference_inputs]
    runtime_closure = _canonical_runtime_manifest_entries(store_entry, runtime_inputs)
    tool_closure = _canonical_tool_manifest_entries(store_entry, tool_inputs)
    reference_closure = _canonical_reference_manifest_entries(store_entry, reference_inputs)
    references = [entry for entry in reference_closure if entry != store_entry]
    if runtime_closure != ctx.attrs.runtime_store_entries:
        fail("pinned package runtime closure does not match manifest metadata for {}".format(logical_store_path))
    if tool_closure != (ctx.attrs.tool_store_entries if ctx.attrs.tool_store_entries else ctx.attrs.runtime_store_entries):
        fail("pinned package tool-use closure does not match manifest metadata for {}".format(logical_store_path))
    if _logical_store_paths(references) != ctx.attrs.references:
        fail("pinned package references do not match manifest metadata for {}".format(logical_store_path))

    return struct(
        reference_closure = reference_closure,
        reference_inputs = reference_inputs,
        runtime_closure = runtime_closure,
        runtime_inputs = runtime_inputs,
        tool_closure = tool_closure,
        tool_inputs = tool_inputs,
    )

def _pinned_import_result(ctx, store_entry, logical_store_path, store_output, metadata):
    runtime_store_outputs = _runtime_store_outputs_for_entries(
        metadata.runtime_closure,
        store_entry,
        store_output,
        metadata.runtime_inputs,
    )
    tool_store_outputs = _tool_store_outputs_for_entries(
        metadata.tool_closure,
        store_entry,
        store_output,
        metadata.tool_inputs,
    )
    reference_store_outputs = _reference_store_outputs_for_entries(
        metadata.reference_closure,
        store_entry,
        store_output,
        metadata.reference_inputs,
    )
    result = [
        DefaultInfo(default_output = store_output),
        PkgsPackageInfo(
            build_closure = metadata.tool_closure,
            foreign_build_entries = [],
            foreign_runtime_entries = [],
            is_foreign = False,
            logical_store_path = logical_store_path,
            name = ctx.attrs.package_name,
            output = ctx.attrs.output,
            reference_closure = metadata.reference_closure,
            reference_store_outputs = reference_store_outputs,
            runtime_closure = metadata.runtime_closure,
            runtime_store_outputs = runtime_store_outputs,
            store_entry = store_entry,
            store_output = store_output,
            store_path_key = ctx.attrs.store_path_key,
            tree = store_output,
            tool_closure = metadata.tool_closure,
            tool_store_outputs = tool_store_outputs,
            validation_outputs = [],
            version = ctx.attrs.version,
        ),
    ]
    pkg_config = _pkg_config_provider_for_path(logical_store_path, ctx.attrs.pkg_config_paths)
    if pkg_config != None:
        result.append(pkg_config)
    return result

def _pkgs_hydrated_store_output_impl(ctx):
    store_entry = "{}-{}".format(ctx.attrs.store_path_key, ctx.attrs.store_name)
    logical_store_path = "{}/{}".format(LOGICAL_STORE_ROOT, store_entry)
    metadata = _pinned_import_metadata(ctx, store_entry, logical_store_path)

    store_path = ctx.actions.store_path(
        store_path_key = ctx.attrs.store_path_key,
        store_name = ctx.attrs.store_name,
    )
    store_output = ctx.actions.import_store_output(
        store_path = store_path,
        dir = True,
        missing_hint = ctx.attrs.missing_hint,
        manifest = ctx.attrs.manifest,
        package_name = ctx.attrs.package_name,
        version = ctx.attrs.version,
        output = ctx.attrs.output,
        target_system = ctx.attrs.target_system,
        references = ctx.attrs.references,
        runtime_store_outputs = [
            "{}/{}".format(LOGICAL_STORE_ROOT, entry)
            for entry in ctx.attrs.runtime_store_entries
        ],
        canonical_tree_hash = ctx.attrs.canonical_tree_hash,
    )
    return _pinned_import_result(ctx, store_entry, logical_store_path, store_output, metadata)

_pkgs_hydrated_store_output = rule(
    impl = _pkgs_hydrated_store_output_impl,
    attrs = {
        "manifest": attrs.source(),
        "canonical_tree_hash": attrs.string(),
        "missing_hint": attrs.string(),
        "output": attrs.string(default = "out"),
        "package_name": attrs.string(),
        "pkg_config_paths": attrs.list(attrs.string(), default = []),
        "references": attrs.list(attrs.string(), default = []),
        "reference_inputs": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "runtime_inputs": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "runtime_store_entries": attrs.list(attrs.string(), default = []),
        "store_name": attrs.string(),
        "store_path_key": attrs.string(),
        "target_system": attrs.string(),
        "tool_inputs": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "tool_store_entries": attrs.list(attrs.string(), default = []),
        "version": attrs.string(),
    },
)

def pkgs_hydrated_store_output(
        name,
        manifest,
        package_name,
        version,
        store_path_key,
        store_name,
        target_system,
        missing_hint,
        canonical_tree_hash,
        output = "out",
        references = [],
        runtime_store_entries = [],
        runtime_inputs = [],
        reference_inputs = [],
        tool_store_entries = [],
        tool_inputs = [],
        visibility = [],
        pkg_config_paths = []):
    _pkgs_hydrated_store_output(
        name = name,
        manifest = manifest,
        canonical_tree_hash = canonical_tree_hash,
        package_name = package_name,
        version = version,
        output = output,
        pkg_config_paths = pkg_config_paths,
        store_path_key = store_path_key,
        store_name = store_name,
        target_system = target_system,
        missing_hint = missing_hint,
        references = references,
        reference_inputs = reference_inputs,
        runtime_store_entries = runtime_store_entries,
        runtime_inputs = runtime_inputs,
        tool_store_entries = tool_store_entries,
        tool_inputs = tool_inputs,
        visibility = visibility,
    )

def _pkgs_cas_store_output_impl(ctx):
    store_entry = "{}-{}".format(ctx.attrs.store_path_key, ctx.attrs.store_name)
    logical_store_path = "{}/{}".format(LOGICAL_STORE_ROOT, store_entry)
    metadata = _pinned_import_metadata(ctx, store_entry, logical_store_path)

    store_path = ctx.actions.store_path(
        store_path_key = ctx.attrs.store_path_key,
        store_name = ctx.attrs.store_name,
    )
    store_output = ctx.actions.import_cas_store_output(
        store_path = store_path,
        dir = True,
        manifest = ctx.attrs.manifest,
        cas_root_digest = ctx.attrs.cas_root_digest,
        re_use_case = ctx.attrs.cas_use_case,
        package_name = ctx.attrs.package_name,
        version = ctx.attrs.version,
        output = ctx.attrs.output,
        target_system = ctx.attrs.target_system,
        references = ctx.attrs.references,
        runtime_store_outputs = [
            "{}/{}".format(LOGICAL_STORE_ROOT, entry)
            for entry in ctx.attrs.runtime_store_entries
        ],
        canonical_tree_hash = ctx.attrs.canonical_tree_hash,
    )
    return _pinned_import_result(ctx, store_entry, logical_store_path, store_output, metadata)

_pkgs_cas_store_output = rule(
    impl = _pkgs_cas_store_output_impl,
    attrs = {
        "manifest": attrs.source(),
        "canonical_tree_hash": attrs.string(),
        "cas_root_digest": attrs.string(),
        "cas_use_case": attrs.string(default = "buckpkgs-published"),
        "output": attrs.string(default = "out"),
        "package_name": attrs.string(),
        "pkg_config_paths": attrs.list(attrs.string(), default = []),
        "references": attrs.list(attrs.string(), default = []),
        "reference_inputs": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "runtime_inputs": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "runtime_store_entries": attrs.list(attrs.string(), default = []),
        "store_name": attrs.string(),
        "store_path_key": attrs.string(),
        "target_system": attrs.string(),
        "tool_inputs": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "tool_store_entries": attrs.list(attrs.string(), default = []),
        "version": attrs.string(),
    },
)

def pkgs_cas_store_output(
        name,
        manifest,
        package_name,
        version,
        store_path_key,
        store_name,
        target_system,
        canonical_tree_hash,
        cas_root_digest,
        output = "out",
        references = [],
        runtime_store_entries = [],
        runtime_inputs = [],
        reference_inputs = [],
        tool_store_entries = [],
        tool_inputs = [],
        cas_use_case = "buckpkgs-published",
        visibility = [],
        pkg_config_paths = []):
    _pkgs_cas_store_output(
        name = name,
        manifest = manifest,
        canonical_tree_hash = canonical_tree_hash,
        cas_root_digest = cas_root_digest,
        cas_use_case = cas_use_case,
        package_name = package_name,
        version = version,
        output = output,
        pkg_config_paths = pkg_config_paths,
        store_path_key = store_path_key,
        store_name = store_name,
        target_system = target_system,
        references = references,
        reference_inputs = reference_inputs,
        runtime_store_entries = runtime_store_entries,
        runtime_inputs = runtime_inputs,
        tool_store_entries = tool_store_entries,
        tool_inputs = tool_inputs,
        visibility = visibility,
    )

def _pkgs_seed_free_impl(ctx):
    packages = [dep[PkgsPackageInfo] for dep in ctx.attrs.packages]
    forbidden = [dep[PkgsPackageInfo] for dep in ctx.attrs.forbidden]

    leaked_runtime_entries = []
    for package in packages:
        leaked_runtime_entries.extend(package.foreign_runtime_entries)
    if leaked_runtime_entries:
        fail("runtime closure still contains foreign entries: {}".format(leaked_runtime_entries))

    stamp = ctx.actions.declare_output("{}.seed_free".format(ctx.label.name))
    args = cmd_args(
        [
            ctx.attrs._verifier[RunInfo],
            "--stamp",
            stamp.as_output(),
        ],
        hidden = [
            output
            for package in packages
            for output in package.validation_outputs
        ],
    )
    for package in packages:
        args.add("--input", package.tree)
    for dep in forbidden:
        args.add("--forbidden", dep.logical_store_path)

    ctx.actions.run(
        args,
        category = "pkgs_verify_seed_free",
        identifier = ctx.label.name,
    )

    return [DefaultInfo(default_output = stamp)]

pkgs_seed_free = rule(
    impl = _pkgs_seed_free_impl,
    attrs = {
        "forbidden": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "packages": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "_verifier": attrs.default_only(
            attrs.exec_dep(
                default = "//crates/pkgs-tool:pkgs_verify_no_refs",
                providers = [RunInfo],
            ),
        ),
    },
)

def _pkgs_seed_cut_impl(ctx):
    package = ctx.attrs.package[PkgsPackageInfo]
    forbidden = [dep[PkgsPackageInfo] for dep in ctx.attrs.forbidden]
    if package.foreign_runtime_entries:
        fail("cannot discharge foreign build inputs while runtime closure still contains foreign entries: {}".format(package.foreign_runtime_entries))

    stamp = ctx.actions.declare_output("{}.seed_cut".format(ctx.label.name))
    args = cmd_args(
        [
            ctx.attrs._verifier[RunInfo],
            "--stamp",
            stamp.as_output(),
            "--input",
            package.tree,
        ],
        hidden = package.validation_outputs,
    )
    for entry in package.foreign_build_entries:
        args.add("--forbidden", "{}/{}".format(LOGICAL_STORE_ROOT, entry))
    for dep in forbidden:
        args.add("--forbidden", dep.logical_store_path)
    ctx.actions.run(
        args,
        category = "pkgs_verify_seed_cut",
        identifier = ctx.label.name,
    )

    result = [
        DefaultInfo(default_output = package.store_output, other_outputs = [stamp]),
        PkgsPackageInfo(
            build_closure = package.build_closure,
            foreign_build_entries = [],
            foreign_runtime_entries = package.foreign_runtime_entries,
            is_foreign = package.is_foreign,
            logical_store_path = package.logical_store_path,
            name = package.name,
            output = package.output,
            reference_closure = package.reference_closure,
            reference_store_outputs = package.reference_store_outputs,
            runtime_closure = package.runtime_closure,
            runtime_store_outputs = package.runtime_store_outputs,
            store_entry = package.store_entry,
            store_output = package.store_output,
            store_path_key = package.store_path_key,
            tree = package.tree,
            tool_closure = package.tool_closure,
            tool_store_outputs = package.tool_store_outputs,
            validation_outputs = package.validation_outputs + [stamp],
            version = package.version,
        ),
    ]
    if PkgConfigInfo in ctx.attrs.package:
        result.append(ctx.attrs.package[PkgConfigInfo])
    return result

pkgs_seed_cut = rule(
    impl = _pkgs_seed_cut_impl,
    attrs = {
        "forbidden": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "package": attrs.dep(providers = [PkgsPackageInfo]),
        "_verifier": attrs.default_only(
            attrs.exec_dep(
                default = "//crates/pkgs-tool:pkgs_verify_no_refs",
                providers = [RunInfo],
            ),
        ),
    },
)

def _pkgs_foreign_build_free_impl(ctx):
    packages = [dep[PkgsPackageInfo] for dep in ctx.attrs.packages]

    leaked_build_entries = []
    for package in packages:
        leaked_build_entries.extend(package.foreign_build_entries)
    if leaked_build_entries:
        fail("build closure still contains foreign entries: {}".format(leaked_build_entries))

    stamp = ctx.actions.declare_output("{}.foreign_build_free".format(ctx.label.name))
    ctx.actions.run(
        cmd_args(
            [
                ctx.attrs._verifier[RunInfo],
                "--stamp",
                stamp.as_output(),
            ],
            hidden = [
                output
                for package in packages
                for output in package.validation_outputs
            ],
        ),
        category = "pkgs_verify_foreign_build_free",
        identifier = ctx.label.name,
    )

    return [DefaultInfo(default_output = stamp)]

pkgs_foreign_build_free = rule(
    impl = _pkgs_foreign_build_free_impl,
    attrs = {
        "packages": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "_verifier": attrs.default_only(
            attrs.exec_dep(
                default = "//crates/pkgs-tool:pkgs_verify_no_refs",
                providers = [RunInfo],
            ),
        ),
    },
)

def _pkgs_elf_interpreters_impl(ctx):
    packages = [dep[PkgsPackageInfo] for dep in ctx.attrs.packages]
    interpreter = ctx.attrs.interpreter[PkgsPackageInfo]
    stamp = ctx.actions.declare_output("{}.elf_interpreters".format(ctx.label.name))
    args = cmd_args([
        ctx.attrs._verifier[RunInfo],
        "--expected-interpreter",
        "{}/{}".format(interpreter.logical_store_path, ctx.attrs.interpreter_relpath),
        "--stamp",
        stamp.as_output(),
    ])
    for runtime in ctx.attrs.allowed_runtime_packages:
        args.add("--allowed-runtime-prefix", runtime[PkgsPackageInfo].logical_store_path)
    for library in ctx.attrs.required_needed:
        args.add("--required-needed", library)
    for library in ctx.attrs.forbidden_needed:
        args.add("--forbidden-needed", library)
    for package in packages:
        args.add("--input", package.tree)

    ctx.actions.run(
        cmd_args(
            args,
            hidden = [
                output
                for package in packages
                for output in package.validation_outputs
            ],
        ),
        category = "pkgs_verify_elf_interpreters",
        identifier = ctx.label.name,
    )

    return [DefaultInfo(default_output = stamp)]

pkgs_elf_interpreters = rule(
    impl = _pkgs_elf_interpreters_impl,
    attrs = {
        "allowed_runtime_packages": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "forbidden_needed": attrs.list(attrs.string(), default = []),
        "interpreter": attrs.dep(providers = [PkgsPackageInfo]),
        "interpreter_relpath": attrs.string(),
        "packages": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "required_needed": attrs.list(attrs.string(), default = []),
        "_verifier": attrs.default_only(
            attrs.exec_dep(
                default = "//crates/pkgs-tool:pkgs_verify_elf_interpreters",
                providers = [RunInfo],
            ),
        ),
    },
)

def _package_with_validation(dep, package, stamps):
    result = [
        DefaultInfo(default_output = package.store_output, other_outputs = stamps),
        PkgsPackageInfo(
            build_closure = package.build_closure,
            foreign_build_entries = package.foreign_build_entries,
            foreign_runtime_entries = package.foreign_runtime_entries,
            is_foreign = package.is_foreign,
            logical_store_path = package.logical_store_path,
            name = package.name,
            output = package.output,
            reference_closure = package.reference_closure,
            reference_store_outputs = package.reference_store_outputs,
            runtime_closure = package.runtime_closure,
            runtime_store_outputs = package.runtime_store_outputs,
            store_entry = package.store_entry,
            store_output = package.store_output,
            store_path_key = package.store_path_key,
            tree = package.tree,
            tool_closure = package.tool_closure,
            tool_store_outputs = package.tool_store_outputs,
            validation_outputs = package.validation_outputs + stamps,
            version = package.version,
        ),
    ]
    if PkgConfigInfo in dep:
        result.append(dep[PkgConfigInfo])
    return result

def _pkgs_output_policy_impl(ctx):
    package = ctx.attrs.package[PkgsPackageInfo]
    stamp = ctx.actions.declare_output("{}.output_policy".format(ctx.label.name))
    args = cmd_args(
        [
            ctx.attrs._verifier[RunInfo],
            "--input",
            package.tree,
            "--stamp",
            stamp.as_output(),
        ],
        hidden = package.validation_outputs,
    )
    for path in ctx.attrs.allowed_runtime_paths:
        args.add("--allow-runtime-path", path)
    ctx.actions.run(
        args,
        category = "pkgs_verify_output_policy",
        identifier = ctx.label.name,
    )
    return _package_with_validation(ctx.attrs.package, package, [stamp])

pkgs_output_policy = rule(
    impl = _pkgs_output_policy_impl,
    attrs = {
        "allowed_runtime_paths": attrs.list(attrs.string(), default = []),
        "package": attrs.dep(providers = [PkgsPackageInfo]),
        "_verifier": attrs.default_only(
            attrs.exec_dep(
                default = "//crates/pkgs-tool:pkgs_verify_output_policy",
                providers = [RunInfo],
            ),
        ),
    },
)

def _pkgs_declared_references_impl(ctx):
    package = ctx.attrs.package[PkgsPackageInfo]
    stamp = ctx.actions.declare_output("{}.declared_references".format(ctx.label.name))
    args = cmd_args(
        [
            ctx.attrs._verifier[RunInfo],
            "--input",
            package.tree,
            "--stamp",
            stamp.as_output(),
        ],
        hidden = package.validation_outputs,
    )
    for entry in package.reference_closure:
        args.add("--declared", "{}/{}".format(LOGICAL_STORE_ROOT, entry))
    ctx.actions.run(
        args,
        category = "pkgs_verify_declared_references",
        identifier = ctx.label.name,
    )
    return _package_with_validation(ctx.attrs.package, package, [stamp])

pkgs_declared_references = rule(
    impl = _pkgs_declared_references_impl,
    attrs = {
        "package": attrs.dep(providers = [PkgsPackageInfo]),
        "_verifier": attrs.default_only(
            attrs.exec_dep(
                default = "//crates/pkgs-tool:pkgs_verify_declared_refs",
                providers = [RunInfo],
            ),
        ),
    },
)

def pkgs_validated_output(
        name,
        package,
        allowed_runtime_paths = [],
        visibility = []):
    policy_name = "{}_payload_policy".format(name)
    pkgs_output_policy(
        name = policy_name,
        package = package,
        allowed_runtime_paths = allowed_runtime_paths,
    )
    pkgs_declared_references(
        name = name,
        package = ":{}".format(policy_name),
        visibility = visibility,
    )

def _pkg_config_search_paths(deps):
    paths = []
    seen = {}
    for dep in deps:
        if PkgConfigInfo in dep:
            for path in dep[PkgConfigInfo].search_paths:
                if path not in seen:
                    paths.append(path)
                    seen[path] = True
    return paths

def _add_pkg_config_args(ctx, args):
    for path in _pkg_config_search_paths(ctx.attrs.build_inputs + getattr(ctx.attrs, "link_inputs", []) + getattr(ctx.attrs, "link_interface_inputs", [])):
        args.add("--pkg-config-path", path)
    for path in _pkg_config_search_paths(ctx.attrs.native_build_inputs):
        args.add("--pkg-config-path-for-build", path)
    for path in _pkg_config_search_paths(ctx.attrs.target_inputs):
        args.add("--pkg-config-path-for-target", path)

def _add_split_output_args(ctx, args, metadata, split_metadata, split_trees):
    for name, paths in sorted(ctx.attrs.split_outputs.items()):
        args.add("--split-output", cmd_args(["{}=".format(name), split_trees[name]], delimiter = ""))
        args.add("--split-output-prefix", "{}={}".format(name, split_metadata[name].logical_store_path))
        for path in paths:
            args.add("--split-path", "{}={}".format(name, path))
    for name, symlinks in sorted(ctx.attrs.split_reference_paths.items()):
        for link, target in sorted(symlinks.items()):
            args.add(
                "--split-reference-symlink",
                "{}={}={}/{}".format(name, link, metadata.logical_store_path, target),
            )
    for name, symlinks in sorted(ctx.attrs.split_reference_output_paths.items()):
        for link, target in sorted(symlinks.items()):
            target_metadata = metadata if target[0] == ctx.attrs.output else split_metadata[target[0]]
            args.add(
                "--split-reference-symlink",
                "{}={}={}/{}".format(name, link, target_metadata.logical_store_path, target[1]),
            )

def _make_install_args(
        ctx,
        source,
        output,
        metadata,
        native_build_inputs,
        link_inputs,
        link_interface_inputs,
        split_metadata = {},
        split_trees = {}):
    self_store_path = metadata.logical_store_path

    args = cmd_args([
        ctx.attrs._builder[RunInfo],
        "--source",
        source,
        "--output",
        output,
        "--install-prefix",
        metadata.logical_store_path,
        "--make-jobs",
        str(ctx.attrs.make_jobs),
    ])
    for dep in native_build_inputs:
        args.add(
            "--path-entry",
            "{}/bin".format(dep.logical_store_path),
        )
    for dep in link_inputs:
        args.add("--link-input", dep.logical_store_path)
    for dep in link_interface_inputs:
        args.add("--link-interface-input", dep.logical_store_path)
    _add_pkg_config_args(ctx, args)
    for path in ctx.attrs.output_paths:
        args.add("--output-path", path)
    for suffix in ctx.attrs.excluded_file_suffixes:
        args.add("--exclude-file-suffix", suffix)
    _add_debug_policy_args(ctx, args)
    _add_split_output_args(ctx, args, metadata, split_metadata, split_trees)
    for arg in ctx.attrs.make_args:
        args.add("--make-arg={}".format(arg))
    for prefix, dep, suffix in ctx.attrs.make_arg_store_paths:
        args.add("--make-arg={}{}{}".format(
            prefix,
            dep[PkgsPackageInfo].logical_store_path,
            suffix,
        ))
    for prefix, suffix in ctx.attrs.make_arg_self_store_paths:
        args.add("--make-arg={}{}{}".format(prefix, self_store_path, suffix))
    for prefix, deps, separator, suffix in ctx.attrs.make_arg_store_path_joins:
        args.add("--make-arg={}{}{}".format(
            prefix,
            separator.join([dep[PkgsPackageInfo].logical_store_path for dep in deps]),
            suffix,
        ))
    for arg in ctx.attrs.install_args:
        args.add("--install-arg={}".format(arg))
    for prefix, dep, suffix in ctx.attrs.install_arg_store_paths:
        args.add("--install-arg={}{}{}".format(
            prefix,
            dep[PkgsPackageInfo].logical_store_path,
            suffix,
        ))
    for prefix, suffix in ctx.attrs.install_arg_self_store_paths:
        args.add("--install-arg={}{}{}".format(prefix, self_store_path, suffix))
    for prefix, deps, separator, suffix in ctx.attrs.install_arg_store_path_joins:
        args.add("--install-arg={}{}{}".format(
            prefix,
            separator.join([dep[PkgsPackageInfo].logical_store_path for dep in deps]),
            suffix,
        ))
    for patch in ctx.attrs.patches:
        args.add("--patch", patch[DefaultInfo].default_outputs[0])
    args.add("--patch-strip", str(ctx.attrs.patch_strip))
    for link, target in sorted(ctx.attrs.symlinks.items()):
        args.add("--symlink", "{}={}".format(link, target))
    _add_python_bytecode_args(ctx, args)
    return args

def _add_python_bytecode_args(ctx, args):
    for directory in getattr(ctx.attrs, "python_bytecode_dirs", []):
        args.add("--python-bytecode-dir", directory)
    for optimization in getattr(ctx.attrs, "python_bytecode_optimizations", []):
        args.add("--python-bytecode-optimize", str(optimization))
    python_bytecode_interpreters = getattr(ctx.attrs, "python_bytecode_interpreters", [])
    if python_bytecode_interpreters:
        args.add(
            "--python-bytecode-interpreter",
            "{}/bin/python3".format(python_bytecode_interpreters[0][PkgsPackageInfo].logical_store_path),
        )
    elif getattr(ctx.attrs, "python_bytecode_dirs", []):
        args.add("--python-bytecode-self-interpreter", "bin/python3")

def _add_debug_policy_args(ctx, args):
    if getattr(ctx.attrs, "preserve_debug", False):
        args.add("--preserve-debug")

def _pkgs_make_install_package_impl(ctx):
    source = ctx.attrs.source[DefaultInfo].default_outputs[0]
    native_build_inputs = [dep[PkgsPackageInfo] for dep in ctx.attrs.native_build_inputs]
    build_inputs = [dep[PkgsPackageInfo] for dep in ctx.attrs.build_inputs]
    link_inputs = [dep[PkgsPackageInfo] for dep in ctx.attrs.link_inputs]
    link_interface_inputs = [dep[PkgsPackageInfo] for dep in ctx.attrs.link_interface_inputs]
    metadata = _package_metadata(ctx)
    split_metadata = _split_output_metadata(ctx, metadata)

    native_runtime_store_outputs = [
        output
        for dep in native_build_inputs
        for output in dep.tool_store_outputs
    ]
    link_runtime_store_outputs = [
        output
        for dep in link_inputs
        for output in dep.runtime_store_outputs
    ]
    python_bytecode_runtime_store_outputs = [
        output
        for dep in ctx.attrs.python_bytecode_interpreters
        for output in dep[PkgsPackageInfo].runtime_store_outputs
    ]
    target_store_outputs = [dep[PkgsPackageInfo].store_output for dep in ctx.attrs.target_inputs]
    ctx.actions.run(
        cmd_args(
            _make_install_args(
                ctx,
                source,
                metadata.store_output.as_output(),
                metadata,
                native_build_inputs,
                link_inputs,
                link_interface_inputs,
                split_metadata = split_metadata,
                split_trees = {
                    output: output_metadata.store_output.as_output()
                    for output, output_metadata in split_metadata.items()
                },
            ),
            hidden = native_runtime_store_outputs + link_runtime_store_outputs + python_bytecode_runtime_store_outputs + target_store_outputs + [dep.store_output for dep in build_inputs + link_interface_inputs],
        ),
        category = "pkgs_make_install",
        identifier = ctx.label.name,
    )

    replay_tree = ctx.actions.declare_output("{}.replay".format(ctx.label.name), dir = True)
    split_replay_trees = {
        output: ctx.actions.declare_output("{}.{}.replay".format(ctx.label.name, output), dir = True)
        for output in ctx.attrs.split_outputs
    }
    ctx.actions.run(
        cmd_args(
            _make_install_args(
                ctx,
                source,
                replay_tree.as_output(),
                metadata,
                native_build_inputs,
                link_inputs,
                link_interface_inputs,
                split_metadata = split_metadata,
                split_trees = {
                    output: tree.as_output()
                    for output, tree in split_replay_trees.items()
                },
            ),
            hidden = native_runtime_store_outputs + link_runtime_store_outputs + python_bytecode_runtime_store_outputs + target_store_outputs + [dep.store_output for dep in build_inputs + link_interface_inputs],
        ),
        category = "pkgs_replay_make_install",
        identifier = ctx.label.name,
    )
    reproducibility_stamp = _declare_reproducibility_stamp(ctx, metadata, replay_tree)
    archive_metadata_stamp = _declare_archive_metadata_stamp(ctx, metadata)
    output_records = {}
    for output, output_metadata in split_metadata.items():
        output_records[output] = struct(
            archive_metadata_stamp = _declare_archive_metadata_stamp(ctx, output_metadata, output = output),
            package = _package_info(ctx, output_metadata.store_output, output_metadata, output),
            pkg_config = _pkg_config_provider(output_metadata, ctx.attrs.split_pkg_config_paths.get(output, [])),
            reproducibility_stamp = _declare_reproducibility_stamp(
                ctx,
                output_metadata,
                split_replay_trees[output],
                output = output,
            ),
        )

    return _package_result(
        ctx,
        metadata.store_output,
        metadata,
        reproducibility_stamp,
        archive_metadata_stamp,
        other_outputs = [
            output_metadata.store_output
            for output_metadata in split_metadata.values()
        ],
        output_records = output_records,
    )

def _split_output_attrs():
    return {
        "split_outputs": attrs.dict(attrs.string(), attrs.list(attrs.string()), default = {}),
        "split_pkg_config_paths": attrs.dict(attrs.string(), attrs.list(attrs.string()), default = {}),
        "split_reference_inputs": attrs.dict(attrs.string(), attrs.list(attrs.dep(providers = [PkgsPackageInfo])), default = {}),
        "split_reference_output_paths": attrs.dict(attrs.string(), attrs.dict(attrs.string(), attrs.tuple(attrs.string(), attrs.string())), default = {}),
        "split_reference_outputs": attrs.dict(attrs.string(), attrs.list(attrs.string()), default = {}),
        "split_reference_paths": attrs.dict(attrs.string(), attrs.dict(attrs.string(), attrs.string()), default = {}),
        "split_runtime_inputs": attrs.dict(attrs.string(), attrs.list(attrs.dep(providers = [PkgsPackageInfo])), default = {}),
        "split_runtime_outputs": attrs.dict(attrs.string(), attrs.list(attrs.string()), default = {}),
        "split_tool_inputs": attrs.dict(attrs.string(), attrs.list(attrs.dep(providers = [PkgsPackageInfo])), default = {}),
        "split_tool_outputs": attrs.dict(attrs.string(), attrs.list(attrs.string()), default = {}),
    }

_pkgs_make_install_package = rule(
    impl = _pkgs_make_install_package_impl,
    attrs = {
        "build_inputs": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "builder": attrs.string(),
        "foreign": attrs.bool(default = False),
        "install_args": attrs.list(attrs.string(), default = []),
        "install_arg_store_paths": attrs.list(
            attrs.tuple(
                attrs.string(),
                attrs.dep(providers = [PkgsPackageInfo]),
                attrs.string(),
            ),
            default = [],
        ),
        "install_arg_self_store_paths": attrs.list(
            attrs.tuple(attrs.string(), attrs.string()),
            default = [],
        ),
        "install_arg_store_path_joins": attrs.list(
            attrs.tuple(
                attrs.string(),
                attrs.list(attrs.dep(providers = [PkgsPackageInfo])),
                attrs.string(),
                attrs.string(),
            ),
            default = [],
        ),
        "link_inputs": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "link_interface_inputs": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "make_arg_store_paths": attrs.list(
            attrs.tuple(
                attrs.string(),
                attrs.dep(providers = [PkgsPackageInfo]),
                attrs.string(),
            ),
            default = [],
        ),
        "make_arg_self_store_paths": attrs.list(
            attrs.tuple(attrs.string(), attrs.string()),
            default = [],
        ),
        "make_arg_store_path_joins": attrs.list(
            attrs.tuple(
                attrs.string(),
                attrs.list(attrs.dep(providers = [PkgsPackageInfo])),
                attrs.string(),
                attrs.string(),
            ),
            default = [],
        ),
        "make_args": attrs.list(attrs.string(), default = []),
        "make_jobs": attrs.int(default = DEFAULT_MAKE_JOBS),
        "native_build_inputs": attrs.list(attrs.exec_dep(providers = [PkgsPackageInfo]), default = []),
        "output": attrs.string(default = "out"),
        "output_paths": attrs.list(attrs.string(), default = []),
        "excluded_file_suffixes": attrs.list(attrs.string(), default = []),
        "preserve_debug": attrs.bool(default = False),
        "package_name": attrs.string(),
        "patch_digests": attrs.list(attrs.string(), default = []),
        "patch_strip": attrs.int(default = 1),
        "patches": attrs.list(attrs.dep(providers = [DefaultInfo]), default = []),
        "python_bytecode_dirs": attrs.list(attrs.string(), default = []),
        "python_bytecode_interpreters": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "python_bytecode_optimizations": attrs.list(attrs.int(), default = []),
        "pkg_config_paths": attrs.list(attrs.string(), default = []),
        "reference_inputs": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "runtime_inputs": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "source": attrs.dep(providers = [DefaultInfo]),
        "source_digests": attrs.list(attrs.string(), default = []),
        "symlinks": attrs.dict(attrs.string(), attrs.string(), default = {}),
        "target_system": attrs.string(),
        "target_inputs": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "tool_inputs": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "version": attrs.string(),
        "_builder": attrs.default_only(
            attrs.exec_dep(
                default = "//crates/pkgs-tool:pkgs_make_install",
                providers = [RunInfo],
            ),
        ),
        "_reproducibility_verifier": attrs.default_only(
            attrs.exec_dep(
                default = "//crates/pkgs-tool:pkgs_verify_reproducible_tree",
                providers = [RunInfo],
            ),
        ),
        "_archive_metadata_verifier": attrs.default_only(
            attrs.exec_dep(
                default = "//crates/pkgs-tool:pkgs_verify_archive_metadata",
                providers = [RunInfo],
            ),
        ),
    } | _split_output_attrs(),
)

def _linux_headers_args(ctx, source, output, native_build_inputs):
    args = cmd_args([
        ctx.attrs._builder[RunInfo],
        "--source",
        source,
        "--output",
        output,
        "--kernel-release",
        ctx.attrs.kernel_release,
        "--make-jobs",
        str(ctx.attrs.make_jobs),
    ])
    for dep in native_build_inputs:
        args.add(
            "--path-entry",
            "{}/bin".format(dep.logical_store_path),
        )
    for arg in ctx.attrs.make_args:
        args.add("--make-arg={}".format(arg))
    return args

def _pkgs_linux_headers_package_impl(ctx):
    source = ctx.attrs.source[DefaultInfo].default_outputs[0]
    native_build_inputs = [dep[PkgsPackageInfo] for dep in ctx.attrs.native_build_inputs]
    metadata = _package_metadata(ctx)

    native_runtime_store_outputs = [
        output
        for dep in native_build_inputs
        for output in dep.tool_store_outputs
    ]

    ctx.actions.run(
        cmd_args(
            _linux_headers_args(
                ctx,
                source,
                metadata.store_output.as_output(),
                native_build_inputs,
            ),
            hidden = native_runtime_store_outputs,
        ),
        category = "pkgs_linux_headers_install",
        identifier = ctx.label.name,
    )

    replay_tree = ctx.actions.declare_output("{}.replay".format(ctx.label.name), dir = True)
    ctx.actions.run(
        cmd_args(
            _linux_headers_args(ctx, source, replay_tree.as_output(), native_build_inputs),
            hidden = native_runtime_store_outputs,
        ),
        category = "pkgs_replay_linux_headers_install",
        identifier = ctx.label.name,
    )
    reproducibility_stamp = _declare_reproducibility_stamp(ctx, metadata, replay_tree)
    archive_metadata_stamp = _declare_archive_metadata_stamp(ctx, metadata)

    return _package_result(
        ctx,
        metadata.store_output,
        metadata,
        reproducibility_stamp,
        archive_metadata_stamp,
    )

_pkgs_linux_headers_package = rule(
    impl = _pkgs_linux_headers_package_impl,
    attrs = {
        "build_inputs": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "builder": attrs.string(),
        "foreign": attrs.bool(default = False),
        "kernel_release": attrs.string(),
        "make_args": attrs.list(attrs.string(), default = []),
        "make_jobs": attrs.int(default = DEFAULT_MAKE_JOBS),
        "native_build_inputs": attrs.list(attrs.exec_dep(providers = [PkgsPackageInfo]), default = []),
        "output": attrs.string(default = "out"),
        "package_name": attrs.string(),
        "runtime_inputs": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "source": attrs.dep(providers = [DefaultInfo]),
        "source_digests": attrs.list(attrs.string(), default = []),
        "target_system": attrs.string(),
        "target_inputs": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "version": attrs.string(),
        "_builder": attrs.default_only(
            attrs.exec_dep(
                default = "//crates/pkgs-tool:pkgs_linux_headers_install",
                providers = [RunInfo],
            ),
        ),
        "_reproducibility_verifier": attrs.default_only(
            attrs.exec_dep(
                default = "//crates/pkgs-tool:pkgs_verify_reproducible_tree",
                providers = [RunInfo],
            ),
        ),
        "_archive_metadata_verifier": attrs.default_only(
            attrs.exec_dep(
                default = "//crates/pkgs-tool:pkgs_verify_archive_metadata",
                providers = [RunInfo],
            ),
        ),
    },
)

def _configure_make_install_args(
        ctx,
        source,
        output,
        metadata,
        native_build_inputs,
        link_inputs,
        link_interface_inputs,
        split_metadata = {},
        split_trees = {}):
    self_store_path = metadata.logical_store_path

    args = cmd_args([
        ctx.attrs._builder[RunInfo],
        "--source",
        source,
        "--output",
        output,
        "--install-prefix",
        metadata.logical_store_path,
        "--make-jobs",
        str(ctx.attrs.make_jobs),
    ])
    for dep in native_build_inputs:
        args.add(
            "--path-entry",
            "{}/bin".format(dep.logical_store_path),
        )
    for dep in link_inputs:
        args.add("--link-input", dep.logical_store_path)
    for dep in link_interface_inputs:
        args.add("--link-interface-input", dep.logical_store_path)
    _add_pkg_config_args(ctx, args)
    for path in ctx.attrs.output_paths:
        args.add("--output-path", path)
    for suffix in ctx.attrs.excluded_file_suffixes:
        args.add("--exclude-file-suffix", suffix)
    _add_debug_policy_args(ctx, args)
    for path in ctx.attrs.normalize_work_dir_text_paths:
        args.add("--normalize-work-dir-text-path", path)
    _add_split_output_args(ctx, args, metadata, split_metadata, split_trees)
    for arg in ctx.attrs.configure_args:
        args.add("--configure-arg={}".format(arg))
    args.add("--configure-program={}".format(ctx.attrs.configure_program))
    args.add("--configure-prefix-arg={}".format(ctx.attrs.configure_prefix_arg))
    if ctx.attrs.out_of_source:
        args.add("--out-of-source")
    for prefix, dep, suffix in ctx.attrs.configure_arg_store_paths:
        args.add("--configure-arg={}{}{}".format(
            prefix,
            dep[PkgsPackageInfo].logical_store_path,
            suffix,
        ))
    for prefix, suffix in ctx.attrs.configure_arg_self_store_paths:
        args.add("--configure-arg={}{}{}".format(prefix, self_store_path, suffix))
    for prefix, deps, separator, suffix in ctx.attrs.configure_arg_store_path_joins:
        args.add("--configure-arg={}{}{}".format(
            prefix,
            separator.join([dep[PkgsPackageInfo].logical_store_path for dep in deps]),
            suffix,
        ))
    for env in ctx.attrs.configure_env:
        args.add("--configure-env={}".format(env))
    for prefix, dep, suffix in ctx.attrs.configure_env_store_paths:
        args.add("--configure-env={}{}{}".format(
            prefix,
            dep[PkgsPackageInfo].logical_store_path,
            suffix,
        ))
    for prefix, suffix in ctx.attrs.configure_env_self_store_paths:
        args.add("--configure-env={}{}{}".format(prefix, self_store_path, suffix))
    for prefix, deps, separator, suffix in ctx.attrs.configure_env_store_path_joins:
        args.add("--configure-env={}{}{}".format(
            prefix,
            separator.join([dep[PkgsPackageInfo].logical_store_path for dep in deps]),
            suffix,
        ))
    for arg in ctx.attrs.make_args:
        args.add("--make-arg={}".format(arg))
    for arg in ctx.attrs.install_args:
        args.add("--install-arg={}".format(arg))
    for prefix, dep, suffix in ctx.attrs.install_arg_store_paths:
        args.add("--install-arg={}{}{}".format(
            prefix,
            dep[PkgsPackageInfo].logical_store_path,
            suffix,
        ))
    for prefix, suffix in ctx.attrs.install_arg_self_store_paths:
        args.add("--install-arg={}{}{}".format(prefix, self_store_path, suffix))
    for prefix, deps, separator, suffix in ctx.attrs.install_arg_store_path_joins:
        args.add("--install-arg={}{}{}".format(
            prefix,
            separator.join([dep[PkgsPackageInfo].logical_store_path for dep in deps]),
            suffix,
        ))
    for patch in ctx.attrs.patches:
        args.add("--patch", patch[DefaultInfo].default_outputs[0])
    args.add("--patch-strip", str(ctx.attrs.patch_strip))
    for link, target in sorted(ctx.attrs.symlinks.items()):
        args.add("--symlink", "{}={}".format(link, target))
    _add_python_bytecode_args(ctx, args)
    return args

def _pkgs_configure_make_install_package_impl(ctx):
    source = ctx.attrs.source[DefaultInfo].default_outputs[0]
    native_build_inputs = [dep[PkgsPackageInfo] for dep in ctx.attrs.native_build_inputs]
    build_inputs = [dep[PkgsPackageInfo] for dep in ctx.attrs.build_inputs]
    link_inputs = [dep[PkgsPackageInfo] for dep in ctx.attrs.link_inputs]
    link_interface_inputs = [dep[PkgsPackageInfo] for dep in ctx.attrs.link_interface_inputs]
    metadata = _package_metadata(ctx)
    split_metadata = _split_output_metadata(ctx, metadata)

    native_runtime_store_outputs = [
        output
        for dep in native_build_inputs
        for output in dep.tool_store_outputs
    ]
    link_runtime_store_outputs = [
        output
        for dep in link_inputs
        for output in dep.runtime_store_outputs
    ]
    python_bytecode_runtime_store_outputs = [
        output
        for dep in ctx.attrs.python_bytecode_interpreters
        for output in dep[PkgsPackageInfo].runtime_store_outputs
    ]
    target_store_outputs = [dep[PkgsPackageInfo].store_output for dep in ctx.attrs.target_inputs]

    ctx.actions.run(
        cmd_args(
            _configure_make_install_args(
                ctx,
                source,
                metadata.store_output.as_output(),
                metadata,
                native_build_inputs,
                link_inputs,
                link_interface_inputs,
                split_metadata = split_metadata,
                split_trees = {
                    output: output_metadata.store_output.as_output()
                    for output, output_metadata in split_metadata.items()
                },
            ),
            hidden = native_runtime_store_outputs + link_runtime_store_outputs + python_bytecode_runtime_store_outputs + target_store_outputs + [dep.store_output for dep in build_inputs + link_interface_inputs],
        ),
        category = "pkgs_configure_make_install",
        identifier = ctx.label.name,
    )

    replay_tree = ctx.actions.declare_output("{}.replay".format(ctx.label.name), dir = True)
    split_replay_trees = {
        output: ctx.actions.declare_output("{}.{}.replay".format(ctx.label.name, output), dir = True)
        for output in ctx.attrs.split_outputs
    }
    ctx.actions.run(
        cmd_args(
            _configure_make_install_args(
                ctx,
                source,
                replay_tree.as_output(),
                metadata,
                native_build_inputs,
                link_inputs,
                link_interface_inputs,
                split_metadata = split_metadata,
                split_trees = {
                    output: tree.as_output()
                    for output, tree in split_replay_trees.items()
                },
            ),
            hidden = native_runtime_store_outputs + link_runtime_store_outputs + python_bytecode_runtime_store_outputs + target_store_outputs + [dep.store_output for dep in build_inputs + link_interface_inputs],
        ),
        category = "pkgs_replay_configure_make_install",
        identifier = ctx.label.name,
    )
    reproducibility_stamp = _declare_reproducibility_stamp(ctx, metadata, replay_tree)
    archive_metadata_stamp = _declare_archive_metadata_stamp(ctx, metadata)
    output_records = {}
    for output, output_metadata in split_metadata.items():
        output_records[output] = struct(
            archive_metadata_stamp = _declare_archive_metadata_stamp(ctx, output_metadata, output = output),
            package = _package_info(ctx, output_metadata.store_output, output_metadata, output),
            pkg_config = _pkg_config_provider(output_metadata, ctx.attrs.split_pkg_config_paths.get(output, [])),
            reproducibility_stamp = _declare_reproducibility_stamp(
                ctx,
                output_metadata,
                split_replay_trees[output],
                output = output,
            ),
        )

    return _package_result(
        ctx,
        metadata.store_output,
        metadata,
        reproducibility_stamp,
        archive_metadata_stamp,
        other_outputs = [
            output_metadata.store_output
            for output_metadata in split_metadata.values()
        ],
        output_records = output_records,
    )

_pkgs_configure_make_install_package = rule(
    impl = _pkgs_configure_make_install_package_impl,
    attrs = {
        "build_inputs": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "builder": attrs.string(),
        "configure_arg_store_paths": attrs.list(
            attrs.tuple(
                attrs.string(),
                attrs.dep(providers = [PkgsPackageInfo]),
                attrs.string(),
            ),
            default = [],
        ),
        "configure_arg_self_store_paths": attrs.list(
            attrs.tuple(attrs.string(), attrs.string()),
            default = [],
        ),
        "configure_arg_store_path_joins": attrs.list(
            attrs.tuple(
                attrs.string(),
                attrs.list(attrs.dep(providers = [PkgsPackageInfo])),
                attrs.string(),
                attrs.string(),
            ),
            default = [],
        ),
        "configure_args": attrs.list(attrs.string(), default = []),
        "configure_program": attrs.string(default = "./configure"),
        "configure_prefix_arg": attrs.string(default = "--prefix="),
        "configure_env": attrs.list(attrs.string(), default = []),
        "configure_env_store_paths": attrs.list(
            attrs.tuple(
                attrs.string(),
                attrs.dep(providers = [PkgsPackageInfo]),
                attrs.string(),
            ),
            default = [],
        ),
        "configure_env_self_store_paths": attrs.list(
            attrs.tuple(attrs.string(), attrs.string()),
            default = [],
        ),
        "configure_env_store_path_joins": attrs.list(
            attrs.tuple(
                attrs.string(),
                attrs.list(attrs.dep(providers = [PkgsPackageInfo])),
                attrs.string(),
                attrs.string(),
            ),
            default = [],
        ),
        "foreign": attrs.bool(default = False),
        "install_args": attrs.list(attrs.string(), default = []),
        "install_arg_store_paths": attrs.list(
            attrs.tuple(
                attrs.string(),
                attrs.dep(providers = [PkgsPackageInfo]),
                attrs.string(),
            ),
            default = [],
        ),
        "install_arg_self_store_paths": attrs.list(
            attrs.tuple(attrs.string(), attrs.string()),
            default = [],
        ),
        "install_arg_store_path_joins": attrs.list(
            attrs.tuple(
                attrs.string(),
                attrs.list(attrs.dep(providers = [PkgsPackageInfo])),
                attrs.string(),
                attrs.string(),
            ),
            default = [],
        ),
        "link_inputs": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "link_interface_inputs": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "make_args": attrs.list(attrs.string(), default = []),
        "make_jobs": attrs.int(default = DEFAULT_MAKE_JOBS),
        "native_build_inputs": attrs.list(attrs.exec_dep(providers = [PkgsPackageInfo]), default = []),
        "out_of_source": attrs.bool(default = False),
        "output": attrs.string(default = "out"),
        "output_paths": attrs.list(attrs.string(), default = []),
        "excluded_file_suffixes": attrs.list(attrs.string(), default = []),
        "preserve_debug": attrs.bool(default = False),
        "normalize_work_dir_text_paths": attrs.list(attrs.string(), default = []),
        "package_name": attrs.string(),
        "patch_digests": attrs.list(attrs.string(), default = []),
        "patch_strip": attrs.int(default = 1),
        "patches": attrs.list(attrs.dep(providers = [DefaultInfo]), default = []),
        "python_bytecode_dirs": attrs.list(attrs.string(), default = []),
        "python_bytecode_interpreters": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "python_bytecode_optimizations": attrs.list(attrs.int(), default = []),
        "runtime_inputs": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "pkg_config_paths": attrs.list(attrs.string(), default = []),
        "reference_inputs": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "source": attrs.dep(providers = [DefaultInfo]),
        "source_digests": attrs.list(attrs.string(), default = []),
        "symlinks": attrs.dict(attrs.string(), attrs.string(), default = {}),
        "target_system": attrs.string(),
        "target_inputs": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "tool_inputs": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "version": attrs.string(),
        "_builder": attrs.default_only(
            attrs.exec_dep(
                default = "//crates/pkgs-tool:pkgs_configure_make_install",
                providers = [RunInfo],
            ),
        ),
        "_reproducibility_verifier": attrs.default_only(
            attrs.exec_dep(
                default = "//crates/pkgs-tool:pkgs_verify_reproducible_tree",
                providers = [RunInfo],
            ),
        ),
        "_archive_metadata_verifier": attrs.default_only(
            attrs.exec_dep(
                default = "//crates/pkgs-tool:pkgs_verify_archive_metadata",
                providers = [RunInfo],
            ),
        ),
    } | _split_output_attrs(),
)

def _meson_install_args(
        ctx,
        source,
        output,
        metadata,
        native_build_inputs,
        link_inputs,
        link_interface_inputs,
        split_metadata = {},
        split_trees = {}):
    self_store_path = metadata.logical_store_path

    args = cmd_args([
        ctx.attrs._builder[RunInfo],
        "--source",
        source,
        "--output",
        output,
        "--install-prefix",
        metadata.logical_store_path,
        "--meson-jobs",
        str(ctx.attrs.meson_jobs),
    ])
    for dep in native_build_inputs:
        args.add(
            "--path-entry",
            "{}/bin".format(dep.logical_store_path),
        )
    for dep in link_inputs:
        args.add("--link-input", dep.logical_store_path)
    for dep in link_interface_inputs:
        args.add("--link-interface-input", dep.logical_store_path)
    _add_pkg_config_args(ctx, args)
    for path in ctx.attrs.output_paths:
        args.add("--output-path", path)
    for suffix in ctx.attrs.excluded_file_suffixes:
        args.add("--exclude-file-suffix", suffix)
    _add_debug_policy_args(ctx, args)
    _add_split_output_args(ctx, args, metadata, split_metadata, split_trees)
    for arg in ctx.attrs.meson_args:
        args.add("--meson-arg={}".format(arg))
    for prefix, dep, suffix in ctx.attrs.meson_arg_store_paths:
        args.add("--meson-arg={}{}{}".format(
            prefix,
            dep[PkgsPackageInfo].logical_store_path,
            suffix,
        ))
    for prefix, suffix in ctx.attrs.meson_arg_self_store_paths:
        args.add("--meson-arg={}{}{}".format(prefix, self_store_path, suffix))
    for prefix, deps, separator, suffix in ctx.attrs.meson_arg_store_path_joins:
        args.add("--meson-arg={}{}{}".format(
            prefix,
            separator.join([dep[PkgsPackageInfo].logical_store_path for dep in deps]),
            suffix,
        ))
    for env in ctx.attrs.meson_env:
        args.add("--meson-env={}".format(env))
    for prefix, dep, suffix in ctx.attrs.meson_env_store_paths:
        args.add("--meson-env={}{}{}".format(
            prefix,
            dep[PkgsPackageInfo].logical_store_path,
            suffix,
        ))
    for prefix, suffix in ctx.attrs.meson_env_self_store_paths:
        args.add("--meson-env={}{}{}".format(prefix, self_store_path, suffix))
    for prefix, deps, separator, suffix in ctx.attrs.meson_env_store_path_joins:
        args.add("--meson-env={}{}{}".format(
            prefix,
            separator.join([dep[PkgsPackageInfo].logical_store_path for dep in deps]),
            suffix,
        ))
    for arg in ctx.attrs.install_args:
        args.add("--install-arg={}".format(arg))
    for prefix, dep, suffix in ctx.attrs.install_arg_store_paths:
        args.add("--install-arg={}{}{}".format(
            prefix,
            dep[PkgsPackageInfo].logical_store_path,
            suffix,
        ))
    for prefix, suffix in ctx.attrs.install_arg_self_store_paths:
        args.add("--install-arg={}{}{}".format(prefix, self_store_path, suffix))
    for prefix, deps, separator, suffix in ctx.attrs.install_arg_store_path_joins:
        args.add("--install-arg={}{}{}".format(
            prefix,
            separator.join([dep[PkgsPackageInfo].logical_store_path for dep in deps]),
            suffix,
        ))
    for patch in ctx.attrs.patches:
        args.add("--patch", patch[DefaultInfo].default_outputs[0])
    args.add("--patch-strip", str(ctx.attrs.patch_strip))
    for link, target in sorted(ctx.attrs.symlinks.items()):
        args.add("--symlink", "{}={}".format(link, target))
    return args

def _pkgs_meson_install_package_impl(ctx):
    source = ctx.attrs.source[DefaultInfo].default_outputs[0]
    native_build_inputs = [dep[PkgsPackageInfo] for dep in ctx.attrs.native_build_inputs]
    build_inputs = [dep[PkgsPackageInfo] for dep in ctx.attrs.build_inputs]
    link_inputs = [dep[PkgsPackageInfo] for dep in ctx.attrs.link_inputs]
    link_interface_inputs = [dep[PkgsPackageInfo] for dep in ctx.attrs.link_interface_inputs]
    metadata = _package_metadata(ctx)
    split_metadata = _split_output_metadata(ctx, metadata)

    native_runtime_store_outputs = [
        output
        for dep in native_build_inputs
        for output in dep.tool_store_outputs
    ]
    link_runtime_store_outputs = [
        output
        for dep in link_inputs
        for output in dep.runtime_store_outputs
    ]
    target_store_outputs = [dep[PkgsPackageInfo].store_output for dep in ctx.attrs.target_inputs]

    ctx.actions.run(
        cmd_args(
            _meson_install_args(
                ctx,
                source,
                metadata.store_output.as_output(),
                metadata,
                native_build_inputs,
                link_inputs,
                link_interface_inputs,
                split_metadata = split_metadata,
                split_trees = {
                    output: output_metadata.store_output.as_output()
                    for output, output_metadata in split_metadata.items()
                },
            ),
            hidden = native_runtime_store_outputs + link_runtime_store_outputs + target_store_outputs + [dep.store_output for dep in build_inputs + link_interface_inputs],
        ),
        category = "pkgs_meson_install",
        identifier = ctx.label.name,
    )

    replay_tree = ctx.actions.declare_output("{}.replay".format(ctx.label.name), dir = True)
    split_replay_trees = {
        output: ctx.actions.declare_output("{}.{}.replay".format(ctx.label.name, output), dir = True)
        for output in ctx.attrs.split_outputs
    }
    ctx.actions.run(
        cmd_args(
            _meson_install_args(
                ctx,
                source,
                replay_tree.as_output(),
                metadata,
                native_build_inputs,
                link_inputs,
                link_interface_inputs,
                split_metadata = split_metadata,
                split_trees = {
                    output: tree.as_output()
                    for output, tree in split_replay_trees.items()
                },
            ),
            hidden = native_runtime_store_outputs + link_runtime_store_outputs + target_store_outputs + [dep.store_output for dep in build_inputs + link_interface_inputs],
        ),
        category = "pkgs_replay_meson_install",
        identifier = ctx.label.name,
    )
    reproducibility_stamp = _declare_reproducibility_stamp(ctx, metadata, replay_tree)
    archive_metadata_stamp = _declare_archive_metadata_stamp(ctx, metadata)
    output_records = {}
    for output, output_metadata in split_metadata.items():
        output_records[output] = struct(
            archive_metadata_stamp = _declare_archive_metadata_stamp(ctx, output_metadata, output = output),
            package = _package_info(ctx, output_metadata.store_output, output_metadata, output),
            pkg_config = _pkg_config_provider(output_metadata, ctx.attrs.split_pkg_config_paths.get(output, [])),
            reproducibility_stamp = _declare_reproducibility_stamp(
                ctx,
                output_metadata,
                split_replay_trees[output],
                output = output,
            ),
        )

    return _package_result(
        ctx,
        metadata.store_output,
        metadata,
        reproducibility_stamp,
        archive_metadata_stamp,
        other_outputs = [
            output_metadata.store_output
            for output_metadata in split_metadata.values()
        ],
        output_records = output_records,
    )

_pkgs_meson_install_package = rule(
    impl = _pkgs_meson_install_package_impl,
    attrs = {
        "build_inputs": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "builder": attrs.string(),
        "foreign": attrs.bool(default = False),
        "install_args": attrs.list(attrs.string(), default = []),
        "install_arg_store_paths": attrs.list(
            attrs.tuple(
                attrs.string(),
                attrs.dep(providers = [PkgsPackageInfo]),
                attrs.string(),
            ),
            default = [],
        ),
        "install_arg_self_store_paths": attrs.list(
            attrs.tuple(attrs.string(), attrs.string()),
            default = [],
        ),
        "install_arg_store_path_joins": attrs.list(
            attrs.tuple(
                attrs.string(),
                attrs.list(attrs.dep(providers = [PkgsPackageInfo])),
                attrs.string(),
                attrs.string(),
            ),
            default = [],
        ),
        "link_inputs": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "link_interface_inputs": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "meson_arg_store_paths": attrs.list(
            attrs.tuple(
                attrs.string(),
                attrs.dep(providers = [PkgsPackageInfo]),
                attrs.string(),
            ),
            default = [],
        ),
        "meson_arg_self_store_paths": attrs.list(
            attrs.tuple(attrs.string(), attrs.string()),
            default = [],
        ),
        "meson_arg_store_path_joins": attrs.list(
            attrs.tuple(
                attrs.string(),
                attrs.list(attrs.dep(providers = [PkgsPackageInfo])),
                attrs.string(),
                attrs.string(),
            ),
            default = [],
        ),
        "meson_args": attrs.list(attrs.string(), default = []),
        "meson_env": attrs.list(attrs.string(), default = []),
        "meson_env_store_paths": attrs.list(
            attrs.tuple(
                attrs.string(),
                attrs.dep(providers = [PkgsPackageInfo]),
                attrs.string(),
            ),
            default = [],
        ),
        "meson_env_self_store_paths": attrs.list(
            attrs.tuple(attrs.string(), attrs.string()),
            default = [],
        ),
        "meson_env_store_path_joins": attrs.list(
            attrs.tuple(
                attrs.string(),
                attrs.list(attrs.dep(providers = [PkgsPackageInfo])),
                attrs.string(),
                attrs.string(),
            ),
            default = [],
        ),
        "meson_jobs": attrs.int(default = DEFAULT_MAKE_JOBS),
        "native_build_inputs": attrs.list(attrs.exec_dep(providers = [PkgsPackageInfo]), default = []),
        "output": attrs.string(default = "out"),
        "output_paths": attrs.list(attrs.string(), default = []),
        "excluded_file_suffixes": attrs.list(attrs.string(), default = []),
        "preserve_debug": attrs.bool(default = False),
        "package_name": attrs.string(),
        "patch_digests": attrs.list(attrs.string(), default = []),
        "patch_strip": attrs.int(default = 1),
        "patches": attrs.list(attrs.dep(providers = [DefaultInfo]), default = []),
        "pkg_config_paths": attrs.list(attrs.string(), default = []),
        "reference_inputs": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "runtime_inputs": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "source": attrs.dep(providers = [DefaultInfo]),
        "source_digests": attrs.list(attrs.string(), default = []),
        "symlinks": attrs.dict(attrs.string(), attrs.string(), default = {}),
        "target_system": attrs.string(),
        "target_inputs": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "tool_inputs": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "version": attrs.string(),
        "_builder": attrs.default_only(
            attrs.exec_dep(
                default = "//crates/pkgs-tool:pkgs_meson_install",
                providers = [RunInfo],
            ),
        ),
        "_reproducibility_verifier": attrs.default_only(
            attrs.exec_dep(
                default = "//crates/pkgs-tool:pkgs_verify_reproducible_tree",
                providers = [RunInfo],
            ),
        ),
        "_archive_metadata_verifier": attrs.default_only(
            attrs.exec_dep(
                default = "//crates/pkgs-tool:pkgs_verify_archive_metadata",
                providers = [RunInfo],
            ),
        ),
    } | _split_output_attrs(),
)

def _cc_wrapper_args(ctx, output, cc, bintools, headers, libc_runtime, libc_dev, gcc_dev, libgcc_runtime, libstdcxx_runtime, shell):
    args = [
        ctx.attrs._builder[RunInfo],
        "--output",
        output,
        "--shell",
        "{}/bin/bash".format(shell.logical_store_path),
        "--cc",
        "{}/bin/gcc".format(cc.logical_store_path),
        "--cxx",
        "{}/bin/g++".format(cc.logical_store_path),
        "--cpp",
        "{}/bin/cpp".format(cc.logical_store_path),
        "--libc-runtime",
        libc_runtime.logical_store_path,
        "--libc-dev",
        libc_dev.logical_store_path,
        "--gcc-dev",
        gcc_dev.logical_store_path,
        "--libgcc-runtime",
        libgcc_runtime.logical_store_path,
        "--libstdcxx-runtime",
        libstdcxx_runtime.logical_store_path,
        "--bintools",
        bintools.logical_store_path,
        "--headers",
        headers.logical_store_path,
        "--dynamic-linker",
        "{}/{}".format(libc_runtime.logical_store_path, ctx.attrs.dynamic_linker),
    ]
    for path in ctx.attrs.cxx_include_paths:
        args.extend([
            "--cxx-include-dir",
            "{}/{}".format(gcc_dev.logical_store_path, path),
        ])
    if ctx.attrs.suppress_cxx_runtime_rpaths:
        args.append("--suppress-cxx-runtime-rpaths")
    return cmd_args(args)

def _pkgs_cc_wrapper_package_impl(ctx):
    cc = ctx.attrs.cc[PkgsPackageInfo]
    bintools = ctx.attrs.bintools[PkgsPackageInfo]
    headers = ctx.attrs.headers[PkgsPackageInfo]
    libc_runtime = ctx.attrs.libc_runtime[PkgsPackageInfo]
    libc_dev = ctx.attrs.libc_dev[PkgsPackageInfo]
    gcc_dev = ctx.attrs.gcc_dev[PkgsPackageInfo]
    libgcc_runtime = ctx.attrs.libgcc_runtime[PkgsPackageInfo]
    libstdcxx_runtime = ctx.attrs.libstdcxx_runtime[PkgsPackageInfo]
    shell = ctx.attrs.shell[PkgsPackageInfo]
    metadata = _package_metadata(ctx)

    ctx.actions.run(
        _cc_wrapper_args(
            ctx,
            metadata.store_output.as_output(),
            cc,
            bintools,
            headers,
            libc_runtime,
            libc_dev,
            gcc_dev,
            libgcc_runtime,
            libstdcxx_runtime,
            shell,
        ),
        category = "pkgs_cc_wrapper_tree",
        identifier = ctx.label.name,
    )

    replay_tree = ctx.actions.declare_output("{}.replay".format(ctx.label.name), dir = True)
    ctx.actions.run(
        _cc_wrapper_args(ctx, replay_tree.as_output(), cc, bintools, headers, libc_runtime, libc_dev, gcc_dev, libgcc_runtime, libstdcxx_runtime, shell),
        category = "pkgs_replay_cc_wrapper_tree",
        identifier = ctx.label.name,
    )
    reproducibility_stamp = _declare_reproducibility_stamp(ctx, metadata, replay_tree)
    archive_metadata_stamp = _declare_archive_metadata_stamp(ctx, metadata)

    return _package_result(
        ctx,
        metadata.store_output,
        metadata,
        reproducibility_stamp,
        archive_metadata_stamp,
    )

_pkgs_cc_wrapper_package = rule(
    impl = _pkgs_cc_wrapper_package_impl,
    attrs = {
        "bintools": attrs.dep(providers = [PkgsPackageInfo]),
        "build_inputs": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "builder": attrs.string(),
        "cc": attrs.dep(providers = [PkgsPackageInfo]),
        "cxx_include_paths": attrs.list(attrs.string(), default = []),
        "dynamic_linker": attrs.string(),
        "foreign": attrs.bool(default = False),
        "headers": attrs.dep(providers = [PkgsPackageInfo]),
        "libc_runtime": attrs.dep(providers = [PkgsPackageInfo]),
        "libc_dev": attrs.dep(providers = [PkgsPackageInfo]),
        "gcc_dev": attrs.dep(providers = [PkgsPackageInfo]),
        "libgcc_runtime": attrs.dep(providers = [PkgsPackageInfo]),
        "libstdcxx_runtime": attrs.dep(providers = [PkgsPackageInfo]),
        "suppress_cxx_runtime_rpaths": attrs.bool(default = False),
        "native_build_inputs": attrs.list(attrs.exec_dep(providers = [PkgsPackageInfo]), default = []),
        "output": attrs.string(default = "bin"),
        "package_name": attrs.string(),
        "reference_inputs": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "runtime_inputs": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "shell": attrs.dep(providers = [PkgsPackageInfo]),
        "target_system": attrs.string(),
        "target_inputs": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "tool_inputs": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "version": attrs.string(),
        "_builder": attrs.default_only(
            attrs.exec_dep(
                default = "//crates/pkgs-tool:pkgs_cc_wrapper_tree",
                providers = [RunInfo],
            ),
        ),
        "_reproducibility_verifier": attrs.default_only(
            attrs.exec_dep(
                default = "//crates/pkgs-tool:pkgs_verify_reproducible_tree",
                providers = [RunInfo],
            ),
        ),
        "_archive_metadata_verifier": attrs.default_only(
            attrs.exec_dep(
                default = "//crates/pkgs-tool:pkgs_verify_archive_metadata",
                providers = [RunInfo],
            ),
        ),
    },
)

def _bintools_wrapper_args(ctx, output, bintools, shell):
    return cmd_args([
        ctx.attrs._builder[RunInfo],
        "--output",
        output,
        "--shell",
        "{}/bin/bash".format(shell.logical_store_path),
        "--binutils",
        bintools.logical_store_path,
    ])

def _pkgs_bintools_wrapper_package_impl(ctx):
    bintools = ctx.attrs.bintools[PkgsPackageInfo]
    shell = ctx.attrs.shell[PkgsPackageInfo]
    metadata = _package_metadata(ctx)

    ctx.actions.run(
        _bintools_wrapper_args(ctx, metadata.store_output.as_output(), bintools, shell),
        category = "pkgs_bintools_wrapper_tree",
        identifier = ctx.label.name,
    )

    replay_tree = ctx.actions.declare_output("{}.replay".format(ctx.label.name), dir = True)
    ctx.actions.run(
        _bintools_wrapper_args(ctx, replay_tree.as_output(), bintools, shell),
        category = "pkgs_replay_bintools_wrapper_tree",
        identifier = ctx.label.name,
    )
    reproducibility_stamp = _declare_reproducibility_stamp(ctx, metadata, replay_tree)
    archive_metadata_stamp = _declare_archive_metadata_stamp(ctx, metadata)

    return _package_result(
        ctx,
        metadata.store_output,
        metadata,
        reproducibility_stamp,
        archive_metadata_stamp,
    )

_pkgs_bintools_wrapper_package = rule(
    impl = _pkgs_bintools_wrapper_package_impl,
    attrs = {
        "bintools": attrs.dep(providers = [PkgsPackageInfo]),
        "build_inputs": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "builder": attrs.string(),
        "foreign": attrs.bool(default = False),
        "native_build_inputs": attrs.list(attrs.exec_dep(providers = [PkgsPackageInfo]), default = []),
        "output": attrs.string(default = "bin"),
        "package_name": attrs.string(),
        "reference_inputs": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "runtime_inputs": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "shell": attrs.dep(providers = [PkgsPackageInfo]),
        "target_system": attrs.string(),
        "target_inputs": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "tool_inputs": attrs.list(attrs.dep(providers = [PkgsPackageInfo]), default = []),
        "version": attrs.string(),
        "_builder": attrs.default_only(
            attrs.exec_dep(
                default = "//crates/pkgs-tool:pkgs_bintools_wrapper_tree",
                providers = [RunInfo],
            ),
        ),
        "_reproducibility_verifier": attrs.default_only(
            attrs.exec_dep(
                default = "//crates/pkgs-tool:pkgs_verify_reproducible_tree",
                providers = [RunInfo],
            ),
        ),
        "_archive_metadata_verifier": attrs.default_only(
            attrs.exec_dep(
                default = "//crates/pkgs-tool:pkgs_verify_archive_metadata",
                providers = [RunInfo],
            ),
        ),
    },
)

def _validate_make_jobs(name, make_jobs):
    if make_jobs < 1:
        fail("package {} must request at least one make job".format(name))

def _validate_python_bytecode(name, directories, interpreter, optimizations):
    if not directories:
        if interpreter != None:
            fail("package {} supplies a Python bytecode interpreter without any bytecode directories".format(name))
        return
    if not optimizations:
        fail("package {} must select at least one Python bytecode optimization level".format(name))
    for optimization in optimizations:
        if optimization < 0 or optimization > 2:
            fail("package {} has invalid Python bytecode optimization level {}".format(name, optimization))

def pkgs_make_install_package(
        name,
        package_name,
        version,
        sources,
        make_args = [],
        make_jobs = DEFAULT_MAKE_JOBS,
        install_args = [],
        patches = [],
        patch_strip = 1,
        symlinks = {},
        output = "out",
        output_paths = [],
        excluded_file_suffixes = [],
        preserve_debug = False,
        native_build_inputs = [],
        build_inputs = [],
        link_inputs = [],
        link_interface_inputs = [],
        runtime_inputs = [],
        reference_inputs = [],
        tool_inputs = [],
        python_bytecode_dirs = [],
        python_bytecode_interpreter = None,
        python_bytecode_optimizations = [0],
        visibility = [],
        pkg_config_paths = [],
        split_outputs = {},
        split_pkg_config_paths = {},
        split_reference_inputs = {},
        split_reference_output_paths = {},
        split_reference_outputs = {},
        split_reference_paths = {},
        split_runtime_inputs = {},
        split_runtime_outputs = {},
        split_tool_inputs = {},
        split_tool_outputs = {}):
    _validate_make_jobs(name, make_jobs)
    _validate_python_bytecode(name, python_bytecode_dirs, python_bytecode_interpreter, python_bytecode_optimizations)
    source = _source_set(name, sources)
    lowered_make_args = _lower_configure_values(make_args)
    lowered_install_args = _lower_configure_values(install_args)
    _pkgs_make_install_package(
        name = name,
        package_name = package_name,
        version = version,
        output = output,
        output_paths = output_paths,
        excluded_file_suffixes = excluded_file_suffixes,
        preserve_debug = preserve_debug,
        builder = "make-install-v17",
        source = source.dep,
        make_args = lowered_make_args.strings,
        make_arg_store_paths = lowered_make_args.store_paths,
        make_arg_self_store_paths = lowered_make_args.self_store_paths,
        make_arg_store_path_joins = lowered_make_args.store_path_joins,
        make_jobs = make_jobs,
        install_args = lowered_install_args.strings,
        install_arg_store_paths = lowered_install_args.store_paths,
        install_arg_self_store_paths = lowered_install_args.self_store_paths,
        install_arg_store_path_joins = lowered_install_args.store_path_joins,
        patches = _select_aware(patches, _descriptor_deps),
        patch_digests = _select_aware(patches, _descriptor_digests),
        patch_strip = patch_strip,
        symlinks = symlinks,
        python_bytecode_dirs = python_bytecode_dirs,
        python_bytecode_interpreters = [] if python_bytecode_interpreter == None else [python_bytecode_interpreter],
        python_bytecode_optimizations = python_bytecode_optimizations if python_bytecode_dirs else [],
        source_digests = source.digests,
        target_system = _TARGET_SYSTEM,
        native_build_inputs = native_build_inputs,
        build_inputs = build_inputs,
        link_inputs = link_inputs,
        link_interface_inputs = link_interface_inputs,
        target_inputs = [],
        reference_inputs = reference_inputs,
        tool_inputs = tool_inputs,
        pkg_config_paths = pkg_config_paths,
        split_outputs = split_outputs,
        split_pkg_config_paths = split_pkg_config_paths,
        split_reference_inputs = split_reference_inputs,
        split_reference_output_paths = split_reference_output_paths,
        split_reference_outputs = split_reference_outputs,
        split_reference_paths = split_reference_paths,
        split_runtime_inputs = split_runtime_inputs,
        split_runtime_outputs = split_runtime_outputs,
        split_tool_inputs = split_tool_inputs,
        split_tool_outputs = split_tool_outputs,
        runtime_inputs = runtime_inputs,
        visibility = visibility,
    )

def pkgs_configure_make_install_package(
        name,
        package_name,
        version,
        sources,
        configure_args = [],
        configure_program = "./configure",
        configure_prefix_arg = "--prefix=",
        configure_env = [],
        out_of_source = False,
        make_args = [],
        make_jobs = DEFAULT_MAKE_JOBS,
        install_args = [],
        patches = [],
        patch_strip = 1,
        symlinks = {},
        output = "out",
        output_paths = [],
        excluded_file_suffixes = [],
        preserve_debug = False,
        normalize_work_dir_text_paths = [],
        native_build_inputs = [],
        build_inputs = [],
        link_inputs = [],
        link_interface_inputs = [],
        runtime_inputs = [],
        reference_inputs = [],
        tool_inputs = [],
        python_bytecode_dirs = [],
        python_bytecode_interpreter = None,
        python_bytecode_optimizations = [0],
        visibility = [],
        pkg_config_paths = [],
        split_outputs = {},
        split_pkg_config_paths = {},
        split_reference_inputs = {},
        split_reference_output_paths = {},
        split_reference_outputs = {},
        split_reference_paths = {},
        split_runtime_inputs = {},
        split_runtime_outputs = {},
        split_tool_inputs = {},
        split_tool_outputs = {}):
    _validate_make_jobs(name, make_jobs)
    _validate_python_bytecode(name, python_bytecode_dirs, python_bytecode_interpreter, python_bytecode_optimizations)
    source = _source_set(name, sources)
    lowered_configure_args = _lower_configure_values(configure_args)
    lowered_configure_env = _lower_configure_values(configure_env)
    lowered_install_args = _lower_configure_values(install_args)
    _pkgs_configure_make_install_package(
        name = name,
        package_name = package_name,
        version = version,
        output = output,
        output_paths = output_paths,
        excluded_file_suffixes = excluded_file_suffixes,
        preserve_debug = preserve_debug,
        normalize_work_dir_text_paths = normalize_work_dir_text_paths,
        builder = "configure-make-install-v18",
        source = source.dep,
        configure_args = lowered_configure_args.strings,
        configure_program = configure_program,
        configure_prefix_arg = configure_prefix_arg,
        configure_arg_store_paths = lowered_configure_args.store_paths,
        configure_arg_self_store_paths = lowered_configure_args.self_store_paths,
        configure_arg_store_path_joins = lowered_configure_args.store_path_joins,
        configure_env = lowered_configure_env.strings,
        configure_env_store_paths = lowered_configure_env.store_paths,
        configure_env_self_store_paths = lowered_configure_env.self_store_paths,
        configure_env_store_path_joins = lowered_configure_env.store_path_joins,
        out_of_source = out_of_source,
        make_args = make_args,
        make_jobs = make_jobs,
        install_args = lowered_install_args.strings,
        install_arg_store_paths = lowered_install_args.store_paths,
        install_arg_self_store_paths = lowered_install_args.self_store_paths,
        install_arg_store_path_joins = lowered_install_args.store_path_joins,
        patches = _select_aware(patches, _descriptor_deps),
        patch_digests = _select_aware(patches, _descriptor_digests),
        patch_strip = patch_strip,
        symlinks = symlinks,
        python_bytecode_dirs = python_bytecode_dirs,
        python_bytecode_interpreters = [] if python_bytecode_interpreter == None else [python_bytecode_interpreter],
        python_bytecode_optimizations = python_bytecode_optimizations if python_bytecode_dirs else [],
        source_digests = source.digests,
        target_system = _TARGET_SYSTEM,
        native_build_inputs = native_build_inputs,
        build_inputs = build_inputs,
        link_inputs = link_inputs,
        link_interface_inputs = link_interface_inputs,
        target_inputs = [],
        reference_inputs = reference_inputs,
        tool_inputs = tool_inputs,
        runtime_inputs = runtime_inputs,
        pkg_config_paths = pkg_config_paths,
        split_outputs = split_outputs,
        split_pkg_config_paths = split_pkg_config_paths,
        split_reference_inputs = split_reference_inputs,
        split_reference_output_paths = split_reference_output_paths,
        split_reference_outputs = split_reference_outputs,
        split_reference_paths = split_reference_paths,
        split_runtime_inputs = split_runtime_inputs,
        split_runtime_outputs = split_runtime_outputs,
        split_tool_inputs = split_tool_inputs,
        split_tool_outputs = split_tool_outputs,
        visibility = visibility,
    )

def pkgs_meson_install_package(
        name,
        package_name,
        version,
        sources,
        meson_args = [],
        meson_env = [],
        meson_jobs = DEFAULT_MAKE_JOBS,
        install_args = [],
        patches = [],
        patch_strip = 1,
        symlinks = {},
        output = "out",
        output_paths = [],
        excluded_file_suffixes = [],
        preserve_debug = False,
        native_build_inputs = [],
        build_inputs = [],
        link_inputs = [],
        link_interface_inputs = [],
        runtime_inputs = [],
        reference_inputs = [],
        tool_inputs = [],
        visibility = [],
        pkg_config_paths = [],
        split_outputs = {},
        split_pkg_config_paths = {},
        split_reference_inputs = {},
        split_reference_output_paths = {},
        split_reference_outputs = {},
        split_reference_paths = {},
        split_runtime_inputs = {},
        split_runtime_outputs = {},
        split_tool_inputs = {},
        split_tool_outputs = {}):
    _validate_make_jobs(name, meson_jobs)
    source = _source_set(name, sources)
    lowered_meson_args = _lower_configure_values(meson_args)
    lowered_meson_env = _lower_configure_values(meson_env)
    lowered_install_args = _lower_configure_values(install_args)
    _pkgs_meson_install_package(
        name = name,
        package_name = package_name,
        version = version,
        output = output,
        output_paths = output_paths,
        excluded_file_suffixes = excluded_file_suffixes,
        preserve_debug = preserve_debug,
        builder = "meson-install-v13",
        source = source.dep,
        meson_args = lowered_meson_args.strings,
        meson_arg_store_paths = lowered_meson_args.store_paths,
        meson_arg_self_store_paths = lowered_meson_args.self_store_paths,
        meson_arg_store_path_joins = lowered_meson_args.store_path_joins,
        meson_env = lowered_meson_env.strings,
        meson_env_store_paths = lowered_meson_env.store_paths,
        meson_env_self_store_paths = lowered_meson_env.self_store_paths,
        meson_env_store_path_joins = lowered_meson_env.store_path_joins,
        meson_jobs = meson_jobs,
        install_args = lowered_install_args.strings,
        install_arg_store_paths = lowered_install_args.store_paths,
        install_arg_self_store_paths = lowered_install_args.self_store_paths,
        install_arg_store_path_joins = lowered_install_args.store_path_joins,
        patches = _select_aware(patches, _descriptor_deps),
        patch_digests = _select_aware(patches, _descriptor_digests),
        patch_strip = patch_strip,
        symlinks = symlinks,
        source_digests = source.digests,
        target_system = _TARGET_SYSTEM,
        native_build_inputs = native_build_inputs,
        build_inputs = build_inputs,
        link_inputs = link_inputs,
        link_interface_inputs = link_interface_inputs,
        target_inputs = [],
        reference_inputs = reference_inputs,
        tool_inputs = tool_inputs,
        pkg_config_paths = pkg_config_paths,
        split_outputs = split_outputs,
        split_pkg_config_paths = split_pkg_config_paths,
        split_reference_inputs = split_reference_inputs,
        split_reference_output_paths = split_reference_output_paths,
        split_reference_outputs = split_reference_outputs,
        split_reference_paths = split_reference_paths,
        split_runtime_inputs = split_runtime_inputs,
        split_runtime_outputs = split_runtime_outputs,
        split_tool_inputs = split_tool_inputs,
        split_tool_outputs = split_tool_outputs,
        runtime_inputs = runtime_inputs,
        visibility = visibility,
    )

def pkgs_linux_headers_package(
        name,
        package_name,
        version,
        sources,
        kernel_release,
        make_args = [],
        make_jobs = DEFAULT_MAKE_JOBS,
        output = "out",
        native_build_inputs = [],
        visibility = []):
    _validate_make_jobs(name, make_jobs)
    source = _source_set(name, sources)
    _pkgs_linux_headers_package(
        name = name,
        package_name = package_name,
        version = version,
        output = output,
        builder = "linux-headers-install-v5",
        source = source.dep,
        kernel_release = kernel_release,
        make_args = make_args,
        make_jobs = make_jobs,
        source_digests = source.digests,
        target_system = _TARGET_SYSTEM,
        native_build_inputs = native_build_inputs,
        visibility = visibility,
    )

def pkgs_cc_wrapper_package(
        name,
        package_name,
        version,
        cc,
        bintools,
        headers,
        libc,
        shell,
        libc_runtime = None,
        libc_dev = None,
        gcc_dev = None,
        cxx_include_paths = [],
        libgcc_runtime = None,
        libstdcxx_runtime = None,
        suppress_cxx_runtime_rpaths = False,
        runtime_inputs = None,
        reference_inputs = None,
        tool_inputs = None,
        output = "bin",
        visibility = []):
    libc_runtime = libc if libc_runtime == None else libc_runtime
    libc_dev = libc if libc_dev == None else libc_dev
    gcc_dev = cc if gcc_dev == None else gcc_dev
    libgcc_runtime = cc if libgcc_runtime == None else libgcc_runtime
    libstdcxx_runtime = cc if libstdcxx_runtime == None else libstdcxx_runtime
    legacy_inputs = [cc, bintools, headers, libc, shell]
    reference_inputs = legacy_inputs if reference_inputs == None else reference_inputs
    runtime_inputs = legacy_inputs if runtime_inputs == None else runtime_inputs
    tool_inputs = reference_inputs if tool_inputs == None else tool_inputs
    builder = "cc-wrapper-tree-v10" if gcc_dev != cc else "cc-wrapper-tree-v8"
    _pkgs_cc_wrapper_package(
        name = name,
        package_name = package_name,
        version = version,
        output = output,
        builder = builder,
        cc = cc,
        bintools = bintools,
        cxx_include_paths = cxx_include_paths,
        dynamic_linker = _DYNAMIC_LINKER,
        headers = headers,
        libc_runtime = libc_runtime,
        libc_dev = libc_dev,
        gcc_dev = gcc_dev,
        libgcc_runtime = libgcc_runtime,
        libstdcxx_runtime = libstdcxx_runtime,
        suppress_cxx_runtime_rpaths = suppress_cxx_runtime_rpaths,
        shell = shell,
        target_system = _TARGET_SYSTEM,
        reference_inputs = reference_inputs,
        runtime_inputs = runtime_inputs,
        tool_inputs = tool_inputs,
        visibility = visibility,
    )

def pkgs_bintools_wrapper_package(
        name,
        package_name,
        version,
        bintools,
        shell,
        runtime_inputs = None,
        reference_inputs = None,
        tool_inputs = None,
        output = "bin",
        visibility = []):
    default_inputs = [bintools, shell]
    reference_inputs = default_inputs if reference_inputs == None else reference_inputs
    runtime_inputs = default_inputs if runtime_inputs == None else runtime_inputs
    tool_inputs = reference_inputs if tool_inputs == None else tool_inputs
    _pkgs_bintools_wrapper_package(
        name = name,
        package_name = package_name,
        version = version,
        output = output,
        builder = "bintools-wrapper-tree-v3",
        bintools = bintools,
        shell = shell,
        target_system = _TARGET_SYSTEM,
        reference_inputs = reference_inputs,
        runtime_inputs = runtime_inputs,
        tool_inputs = tool_inputs,
        visibility = visibility,
    )
